...
const pages = [{pageNumber:1,pageWidth:595,pageHeight:842},{pageNumber:2,pageWidth:400,pageHeight:600}];
test('crop values must be whole-number points, including on fractional PDF pages', () => {
  ...
  for (const key of ['cropX', 'cropY', 'cropWidth', 'cropHeight']) {
    for (const value of [1.5, '1.5']) assert.throws(() => cropPlan({ ...whole, [key]: value }, fractionalPages), /whole-number/);
  }
});
...
test('crop plan validates the exact selected page geometry', () => {
  ...
  for(const update of [{cropWidth:0},{cropHeight:-1},{cropX:Infinity},{cropY:-1},{cropX:401},{cropWidth:500},{pages:'3'},{pages:''},{cropWidth:'x'}]) {
