...
const hooks = registerHooks({
  resolve(specifier, context, next) {
    if (isPage(context.parentURL)) {
      ...
      if (specifier === "@/lib/admin/access") return stub('export async function requirePagePermission(...args){const s=globalThis.__blogPreviewTest;s.calls.push(["permission",...args]);if(s.denied)throw new Error("DENIED");return {user:{id:"admin"}};}');
      if (specifier === "@/lib/blog/queries") return stub(['getBlogPost', 'getBlogRevision'].map(name => `export async function ${name}(...args){const s=globalThis.__blogPreviewTest;s.calls.push(["${name}",...args]);return s.result;}`).join('\n'));
      ...
      if (specifier === "@/components/blog/BlogArticle") return stub('export function BlogArticle({document}){return document.title}');
    }
...
test.after(() => { hooks.deregister(); delete globalThis.__blogPreviewTest; });
function reset(result = null) { state.calls = []; state.denied = false; state.result = result; }
const props = (id = "post-1", revision) => ({ params: Promise.resolve({ id }), searchParams: Promise.resolve({ revision }) });

test("private preview checks permission before any content read and stays non-indexable", async () => {
  reset(); state.denied = true;
