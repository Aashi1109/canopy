...
const element = (color, style = '') => ({ getAttribute: name => name === 'data-color' ? color : null, style: { backgroundColor: style } });
...
test('native highlight commands apply, recolor without duplicates, and remove only the selected text', () => {
  const editor = new Editor({ element: null, extensions, content: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'Hello world' }] }] } });
...
test('native highlights at the caret apply to typing and clear for following text', () => {
  const editor = new Editor({ element: null, extensions, content: { type: 'doc', content: [{ type: 'paragraph' }] } });
