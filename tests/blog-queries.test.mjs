...
import {
  decodeBlogCursor, encodeBlogCursor, paginateBlogRows,
  getBlogPost, getBlogRevision, getPublishedBlogPost,
  listBlogPosts, listBlogRevisions, listBlogTaxonomy,
  listPublishedBlogPosts, listPublishedBlogTaxonomy,
...
test("blog date cursors retain database microseconds and reject invalid or mismatched cursors", () => {
  ...
  for (const value of ["2026-02-30T10:00:00.123456Z", "2026-09-16T10:00:00.123Z", "invalid", "0000-01-01T00:00:00.000000Z"]) {
    assert.throws(() => encodeBlogCursor({ ...cursor, value }));
  }
  assert.throws(() => encodeBlogCursor({ ...cursor, id: "' OR true; --" }));
  assert.throws(() => decodeBlogCursor(Buffer.from(JSON.stringify({ ...cursor, extra: true })).toString("base64url"), "published"), /cursor/);
});

test("keyset pagination emits a next cursor only when a further row exists", () => {
  const rows = Array.from({ length: 13 }, (_, index) => ({ id: `post-${index}`, time: "2026-09-16T10:00:00.000001Z" }));
