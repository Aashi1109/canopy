...
test("Resize PDF Pages previews the source and resizes only selected pages from its settings panel", async ({
  page,
}, testInfo) => {
  ...
  const download = settings.getByRole("button", {
    name: "Download source-resized.pdf",
    exact: true,
  });
  ...
  await expect
    .poll(() =>
      secondPage.evaluate((image: HTMLImageElement) => {
  ...
  expect(result.getPages().map((sheet) => [sheet.getWidth(), sheet.getHeight()])).toEqual([
    [210, 300],
    [792, 612],
    [230, 300],
  ]);
  ...
  const replacementDownload = settings.getByRole("button", {
    name: "Download replacement-resized.pdf",
    exact: true,
  });
