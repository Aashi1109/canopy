...
test("Delete PDF Pages previews the generated remaining pages and clears stale output after changes", async ({ page }, testInfo) => {
  ...
  for (let number = 1; number <= 3; number++) {
    ...
    sheet.drawRectangle({ x: 0, y: 0, width: sheet.getWidth(), height: 300, color: colors[number - 1] });
    sheet.drawText(`Source page ${number}`, { x: 20, y: 150, size: 18, color: rgb(1, 1, 1) });
  }
  const sourceFile = { name: "source.pdf", mimeType: "application/pdf", buffer: Buffer.from(await sourceDocument.save()) };
  ...
  await expect(source.getByRole("button", { name: "Select page 2", exact: true })).toBeVisible({ timeout: 60_000 });
  ...
  await expect(source.getByRole("button", { name: "Deselect page 2", exact: true })).toHaveAttribute("aria-pressed", "true");
  ...
  await output.screenshot({ path: `/tmp/delete-pdf-single-generated-${testInfo.project.name}.png` });
