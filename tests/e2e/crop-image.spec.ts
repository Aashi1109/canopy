...
test("freeform crop supports independent drag, keyboard, recovery and transparent PNG output", async ({ page }) => {
  ...
  await page.mouse.move(moved.x + 22, moved.y + 22);
  await page.mouse.down();
  ...
  const downloadButton = page.getByRole("button", {
    name: "Download crop-test-cropped.png",
    exact: true,
  });
  ...
  const pixels = await page.evaluate(async (base64) => {
    ...
    return {
      width: image.width,
      height: image.height,
      outside: [...ctx.getImageData(390, 10, 1, 1).data],
      inside: [...ctx.getImageData(100, 100, 1, 1).data],
    };
...
test.describe("mobile touch", () => {
  ...
  test("mobile editor can drag, reach actions, return to its selection and download", async ({ page }) => {
    ...
    for (let step = 1; step <= 5; step++)
      await session.send("Input.dispatchTouchEvent", {
        type: "touchMove",
        touchPoints: [{ x: box.x + 22 - step * 6, y: box.y + 22 + step * 6 }],
      });
