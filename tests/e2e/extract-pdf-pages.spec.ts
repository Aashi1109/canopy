...
test("Extract PDF Pages selects and reorders real pages in one source workspace with complete recovery", async ({
  page,
}, testInfo) => {
  ...
  const download = settings.getByRole("button", {
    name: "Download source-extracted.pdf",
    exact: true,
  });
  ...
  const screenshot = async (state: string) => {
    ...
    await page
      ...
      .screenshot({ path: `/tmp/extract-pdf-pages-${state}-${testInfo.project.name}.png` });
  };
  async function expectExtracted(selectedPages: number[]) {
    ...
    expect(extracted.getPages().map((sheet) => sheet.getWidth())).toEqual(
      selectedPages.map((number) => 200 + number * 10),
    );
  ...
  if (testInfo.project.use.hasTouch) {
    ...
    for (let step = 1; step <= 5; step++)
      await cdp.send("Input.dispatchTouchEvent", {
        type: "touchMove",
        touchPoints: [{ x, y: y + step * 30, id: 1 }],
      });
  ...
  await expect(download).toHaveCount(0);
  await expect(source.getByRole("button", { name: "Select page 2", exact: true })).toHaveAttribute(
    "aria-pressed",
    "false",
  );
  ...
  const replacementDownload = settings.getByRole("button", {
    name: "Download replacement-extracted.pdf",
    exact: true,
  });
  ...
  expect(
    (await PDFDocument.load(await readFile((await replacementArtifact.path())!)))
      .getPages()
      .map((sheet) => sheet.getWidth()),
  ).toEqual([250]);
