...
test("a regular account shares its session but cannot enter Admin", async ({ page }) => {
  await new AuthPage(page).signIn(E2E_ACCOUNTS.user.email, E2E_PASSWORD, "http://localhost:3000/admin");
...
test("a custom role combines explicit grants and denies missing ones", async ({ page }) => {
  await new AuthPage(page).signIn(E2E_ACCOUNTS.viewer.email, E2E_PASSWORD, "http://localhost:3000/admin/tools");
...
test("the Admin header highlights the current section", async ({ page }) => {
  ...
  await expect(navigation.getByRole("link", { name: "Templates" })).toHaveAttribute("aria-current", "page");
  ...
  await expect(navigation.getByRole("link", { name: "Roles" })).toHaveAttribute("aria-current", "page");
  await expect(navigation.getByRole("link", { name: "Templates" })).not.toHaveAttribute("aria-current");
});
...
test("an Admin can disable a tool and its direct route is blocked", async ({ page }, testInfo) => {
  ...
  await new AuthPage(page).signIn(E2E_ACCOUNTS.admin.email, E2E_PASSWORD, "http://localhost:3000/admin/tools");
