...
test("Split PDF keeps one source workspace through validation, all split modes and downloads", async ({ page }, testInfo) => {
  ...
  const archiveDownload = settings.getByRole("button", { name: "Download ZIP source-split.zip", exact: true });
  ...
  const screenshot = async (state: string) => {
    ...
    await page.getByTestId("tool-workspace-content").screenshot({ path: `/tmp/split-pdf-${state}-${testInfo.project.name}.png` });
  };
  ...
  await (await chooserEvent).setFiles({ name: "replacement.pdf", mimeType: "application/pdf", buffer: Buffer.from(await replacement.save()) });
  ...
  await expect(downloads).toHaveCount(0);
  await split.click();
  const replacementDownload = settings.getByRole("button", { name: "Download replacement-part-01.pdf", exact: true });
...
test("Split PDF rejects invalid input and preserves a valid source after rejected replacements", async ({ page }, testInfo) => {
  ...
  await page.locator('input[type="file"]').first().setInputFiles({ name: "empty.pdf", mimeType: "application/pdf", buffer: Buffer.alloc(0) });
  await expect(source.getByText("Unable to open PDF", { exact: true })).toBeVisible({ timeout: 60_000 });
  await expect(split).toBeDisabled();
  ...
  await expect(split).toBeDisabled();
  await replaceFile({ name: "corrupt.pdf", mimeType: "application/pdf", buffer: Buffer.from("%PDF-1.7\nThis is not a PDF document.") });
  ...
  await page.screenshot({ path: `/tmp/split-pdf-rejected-${testInfo.project.name}.png`, fullPage: true });

  const validPdf = await PDFDocument.create();
  validPdf.addPage([200, 300]).drawText("Valid replacement", { x: 20, y: 150, size: 14 });
  const validFile = { name: "valid.pdf", mimeType: "application/pdf", buffer: Buffer.from(await validPdf.save()) };
  ...
  await replaceFile({ name: "notes.txt", mimeType: "text/plain", buffer: Buffer.from("Keep the valid PDF when this replacement is rejected.") });
  ...
  const transfer = await page.evaluateHandle((bytes) => {
    ...
    for (const name of ["first.pdf", "second.pdf"]) dataTransfer.items.add(new File([new Uint8Array(bytes)], name, { type: "application/pdf" }));
    return dataTransfer;
  }, [...validFile.buffer]);
  ...
  await replaceFile({ name: "501-pages.pdf", mimeType: "application/pdf", buffer: Buffer.from(await oversizedPdf.save()) });
  await expect(source.getByText("Unable to open PDF", { exact: true })).toBeVisible({ timeout: 60_000 });
