...
test("page cards preview on click and reorder from the whole card without opening the preview", async ({
  page,
}, testInfo) => {
  ...
  const screenshot = async (state: string) => {
    ...
    await page.screenshot({
      path: `/tmp/reorder-pdf-${state}-${testInfo.project.name}.png`,
      fullPage: true,
    });
  };
      ...
      .poll(() =>
        image.evaluate(
          (node: HTMLImageElement) =>
            node.naturalWidth >= Math.floor(node.getBoundingClientRect().width * devicePixelRatio),
        ),
      )
      .toBe(true);
  ...
  if (testInfo.project.name === "desktop") {
    ...
    expect((await output.boundingBox())!.y).toBeGreaterThanOrEqual(
      (await selectedFiles.boundingBox())!.y + (await selectedFiles.boundingBox())!.height,
    );
  }
...
test("one-page PDFs still open full preview when reordering is disabled", async ({
  ...
  await page
    .locator('input[type="file"]')
    .first()
    .setInputFiles(await sourcePdf(1));
  ...
  await page.getByRole("button", { name: "Reorder pages", exact: true }).click();
  await expect(output.getByRole("button", { name: "Download file", exact: true })).toBeVisible({
    timeout: 60_000,
  });
...
test("touch tap previews and a held card drags while a quick swipe scrolls", async ({
  ...
  for (let step = 1; step <= 10; step++) {
    await touch(
      "touchMove",
      from.x + from.width / 2 + ((to.x - from.x) * step) / 10,
