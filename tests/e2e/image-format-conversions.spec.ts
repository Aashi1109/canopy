...
const previewButton = (scope: Page | Locator, name: string) =>
  scope
    .getByRole("button", {
...
async function fixture(page: Page, format: Format, index: number) {
  if (format === "heic") {
    ...
    return {
      name: `source-${index}.heic`,
      mimeType: mime(format),
      buffer: await readFile(process.env.MEDIA_E2E_HEIC_FIXTURE!),
    };
  }
  ...
  test.describe(title, () => {
    ...
    test("real previews, removal, artifact downloads, edit and reset complete the batch flow", async ({
      ...
      for (const input of inputs) {
        const card = page
          .getByRole("article")
          .filter({ has: page.getByRole("button", { name: `Remove ${input.name}`, exact: true }) });
      ...
      for (const [index, name] of names.entries()) {
        const card = gallery
          .getByRole("article")
          .filter({ has: page.getByRole("button", { name: `Download ${name}`, exact: true }) });
        await card.scrollIntoViewIfNeeded();
        const image = card.locator("img");
        await loadedImage(image);
        ...
        const previewMatches = await image.evaluate(
          async (node: HTMLImageElement, bytes) => {
      ...
      const fullScreenDownload = await download(
        page,
        dialog.getByRole("button", { name: `Download ${names[0]}`, exact: true }),
      );
      ...
      for (const input of inputs.slice(0, 2))
        await loadedImage(
          page
            .getByRole("article")
            .filter({
              has: page.getByRole("button", { name: `Remove ${input.name}`, exact: true }),
            })
            .locator("img"),
        );
      await expect(
...
test("PNG settings validation, single-file download and recovery preserve the source", async ({
  ...
  const output = await download(
    page,
    results.getByRole("button", { name: "Download source-1-converted.jpg", exact: true }),
  );
...
test("rejected intake and damaged-image failure keep recovery available", async ({ page }) => {
  ...
  await page
    ...
    .setInputFiles({
      name: "damaged.png",
      mimeType: "image/png",
      buffer: Buffer.from("Not valid PNG bytes"),
    });
  await convert.click();
...
test("output format dropdown keeps inputs, remembers settings and runs the selected encoder", async ({
  ...
  const output = await download(
    page,
    results.getByRole("button", { name: "Download source-1-converted.webp", exact: true }),
  );
