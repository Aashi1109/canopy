...
test('crop PDF supports exact editing, direct cropping, output preview, download and recovery', async ({page}, info) => {
  test.setTimeout(180_000);
  const pdf = await PDFDocument.create();
  for(let i=1;i<=4;i++) pdf.addPage([595.2756,841.8898]).drawText(`Page ${i}`,{x:60,y:740});
  const file={name:'crop-source.pdf',mimeType:'application/pdf',buffer:Buffer.from(await pdf.save())};
  ...
  await navigation.fill('2'); await navigation.press('Enter');
  ...
  await right.press('ArrowRight'); await expect(width).toHaveValue('523');
  await width.fill('900'); await expect(review).toBeDisabled();
  await width.fill('523'); await expect(review).toBeEnabled();
  ...
  await review.click();
  const download=page.getByRole('button',{name:'Download crop-source-cropped.pdf',exact:true});
  await expect(download).toBeVisible({timeout:60000});
  await expect(width).toHaveValue('523');
  await expect(page.getByRole('img',{name:'Generated PDF page 1',exact:true})).toBeVisible({timeout:60000});
  await expect(review).toHaveCount(0);
  const outputNavigation=page.getByRole('spinbutton',{name:'Current page',exact:true});
  await outputNavigation.fill('2'); await outputNavigation.press('Enter');
  ...
  for(const sheet of result.getPages().slice(1)) expect(sheet.getCropBox()).toEqual({x:36,y:36,width:523,height:770});
...
test('crop preview recovers from an invalid file and reseeds replacement geometry', async ({page}) => {
  ...
  await page.goto('/media/crop-pdf');
  await page.waitForLoadState('networkidle');
  await page.locator('input[type=file]').first().setInputFiles({name:'broken.pdf',mimeType:'application/pdf',buffer:Buffer.from('Not a PDF')});
  await expect(page.getByText('Unable to open PDF',{exact:true})).toBeVisible({timeout:60000});
  await expect(page.getByRole('button',{name:'Crop PDF',exact:true})).toBeDisabled();
  const document=await PDFDocument.create(); document.addPage([400,600]);
  const replace=async (pdf:PDFDocument,name:string) => {
    await page.locator('input[type=file]').first().setInputFiles({name,mimeType:'application/pdf',buffer:Buffer.from(await pdf.save())});
  };
  await replace(document,'small.pdf');
  const width=page.getByRole('spinbutton',{name:'Width',exact:true});
  await expect(width).toHaveValue('400');
  await width.fill('250');
  const bigger=await PDFDocument.create(); bigger.addPage([595,842]); bigger.addPage([500,700]);
