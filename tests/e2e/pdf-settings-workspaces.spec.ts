...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    window.addEventListener("test:hold-pdf-run", () => { hold = true; });
    window.addEventListener("test:release-pdf-run", () => { hold = false; release?.(); release = undefined; });
    Worker.prototype.postMessage = function (message: unknown, options?: Transferable[] | StructuredSerializeOptions) {
      ...
      if (hold && typeof message === "object" && message !== null && "type" in message && message.type === "run") { release = dispatch; return; }
      dispatch();
    };
  });
});
...
async function sourcePdf(name = "source.pdf") {
  ...
  for (let number = 1; number <= 2; number++) pdf.addPage([220, 300]).drawText(`Source page ${number}`, { x: 20, y: 150, size: 16 });
...
async function attach(page: Page, tool: string, optionsTitle: string, action: string, project: string) {
  ...
  await page.locator('input[type="file"]').first().setInputFiles(await sourcePdf());
  await expect(source.getByRole("spinbutton", { name: "Current page", exact: true })).toHaveAttribute("max", "2", { timeout: 60_000 });
...
async function runAndDownload(page: Page, settings: Locator, action: string, name: string) {
  ...
  if (page.viewportSize()!.width < 600) {
    ...
    expect((await settings.getByRole("button", { name: action, exact: true }).boundingBox())!.height).toBeGreaterThanOrEqual(44);
  }
...
test("Compress PDF keeps preservation and acknowledged strong compression in its settings panel", async ({ page }, testInfo) => {
  test.setTimeout(180_000);
  const { source, settings } = await attach(page, "compress-pdf", "Compression settings", "Compress PDF", testInfo.project.name);
  ...
  const acknowledge = settings.getByRole("switch", { name: "I understand document content will be flattened", exact: true });
...
test("Watermark PDF supports text and replaceable images while preserving the image on PDF replacement", async ({ page }, testInfo) => {
  test.setTimeout(180_000);
  const { source, settings } = await attach(page, "watermark-pdf", "Watermark settings", "Apply watermark", testInfo.project.name);
  ...
  const select = (scope: Locator, number: number) => scope.getByRole("button", { name: new RegExp(`^(Select|Deselect) page ${number}$`) });
  ...
  await page.screenshot({ path: `/tmp/watermark-pdf-pages-dropdown-${testInfo.project.name}.png`, animations: "disabled" });
  ...
  await page.screenshot({ path: `/tmp/watermark-pdf-pages-custom-${testInfo.project.name}.png`, animations: "disabled" });
  await pageMode.click();
  ...
  await page.screenshot({ path: `/tmp/watermark-pdf-fullscreen-selected-${testInfo.project.name}.png`, animations: "disabled" });
  ...
  await settings.getByLabel("Replace watermark image", { exact: true }).setInputFiles({ ...logo, name: "replacement-logo.png" });
...
test("Add Page Numbers produces real numbering from the configured starting number", async ({ page }, testInfo) => {
  ...
  const { source, settings } = await attach(page, "add-page-numbers", "Page number settings", "Add page numbers", testInfo.project.name);
