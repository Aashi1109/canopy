...
test("blog migration enforces article identity, revision ownership, schedules, and retention", {
  ...
  await sql.unsafe(await readFile(new URL("../packages/database/drizzle/0001_auth_control_plane.sql", import.meta.url), "utf8"));
  ...
  await context.test("migration grants system admin access without changing custom roles or removing protection", async () => {
  ...
  await context.test("slugs are unique forever and cannot change with draft edits", async () => {
    ...
    await assert.rejects(sql`INSERT INTO blog_posts (id, slug, draft_document, draft_hash) VALUES ('duplicate', 'second-post', '{}', 'hash')`, { code: "23505" });
    await sql`UPDATE blog_posts SET trashed_at = NULL WHERE id = 'post-b'`;
    await assert.rejects(sql`INSERT INTO blog_posts (id, slug, draft_document, draft_hash) VALUES ('bad-slug', 'UPPER CASE', '{}', 'hash')`, { code: "23514" });
  });

  await context.test("snapshots and publication are constrained to their owning post", async () => {
    await assert.rejects(sql`UPDATE blog_posts SET published_revision_id = 'revision-b', published_category_id = 'category', first_published_at = NOW(), published_updated_at = NOW() WHERE id = 'post-a'`, { code: "23503" });
    await assert.rejects(sql`INSERT INTO blog_revisions (id, post_id, revision_number, document, content_hash, reason, source_revision_id) VALUES ('bad-restore', 'post-a', 2, '{}', 'hash', 'restore', 'revision-b')`, { code: "23503" });
    await assert.rejects(sql`UPDATE blog_posts SET published_revision_id = 'revision-a' WHERE id = 'post-a'`, { code: "23514" });
    await assert.rejects(sql`UPDATE blog_posts SET draft_document = '[]' WHERE id = 'post-a'`, { code: "23514" });
    await assert.rejects(sql`INSERT INTO blog_revisions (id, post_id, revision_number, document, content_hash, reason) VALUES ('bad-doc', 'post-a', 2, '[]', 'hash', 'create')`, { code: "23514" });
    await assert.rejects(sql`INSERT INTO blog_revisions (id, post_id, revision_number, document, content_hash, reason) VALUES ('bad-number', 'post-a', 1, '{}', 'hash', 'create')`, { code: "23505" });
    ...
    await assert.rejects(sql`UPDATE blog_posts SET trashed_at = NOW() WHERE id = 'post-a'`, { code: "23514" });
  ...
  await context.test("taxonomy names are shared and case-insensitively unique", async () => {
    await assert.rejects(sql`INSERT INTO blog_categories (id, name, slug) VALUES ('duplicate-category', 'GUIDES', 'other-guides')`, { code: "23505" });
    await assert.rejects(sql`INSERT INTO blog_tags (id, name, slug) VALUES ('duplicate-tag', 'pdf', 'other-pdf')`, { code: "23505" });
  ...
  await context.test("schedules freeze a same-post revision and permit only one active request", async () => {
    await assert.rejects(sql`INSERT INTO blog_post_schedules (id, post_id, revision_id, scheduled_at) VALUES ('bad-schedule', 'post-a', 'revision-b', NOW())`, { code: "23503" });
    const results = await Promise.allSettled(["schedule-a", "schedule-b"].map((id) => sql`INSERT INTO blog_post_schedules (id, post_id, revision_id, scheduled_at, scheduled_by) VALUES (${id}, 'post-a', 'revision-a', NOW(), 'author')`));
  ...
  await context.test("account deletion clears attribution without removing articles, terms, revisions, or schedules", async () => {
    ...
    const [schedule] = await sql`SELECT scheduled_by, revision_id FROM blog_post_schedules WHERE post_id = 'post-a'`;
    assert.deepEqual(schedule, { scheduled_by: null, revision_id: "revision-a" });
  });

  await context.test("rerunning the migration preserves content, schedules, grants, and trigger enforcement", async () => {
