...
test(
  ...
  async (context) => {
    ...
    const migration = await readFile(
      new URL("../packages/database/drizzle/0006_blogs.sql", import.meta.url),
      "utf8",
    );
    await sql.unsafe(
      await readFile(
        new URL("../packages/database/drizzle/0001_auth_control_plane.sql", import.meta.url),
        "utf8",
      ),
    );
    ...
    await context.test(
      ...
      async () => {
        ...
        await assert.rejects(
          sql`UPDATE roles SET name = 'Changed' WHERE id = 'admin'`,
    ...
    await context.test("slugs are unique forever and cannot change with draft edits", async () => {
      await assert.rejects(
        sql`UPDATE blog_posts SET slug = 'changed' WHERE id = 'post-a'`,
        /slug is immutable/i,
      );
    ...
    await context.test(
      "snapshots and publication are constrained to their owning post",
      async () => {
        await assert.rejects(
        ...
        await assert.rejects(
          sql`UPDATE blog_posts SET published_revision_id = 'revision-a' WHERE id = 'post-a'`,
          { code: "23514" },
        );
    ...
    await context.test("taxonomy names are shared and case-insensitively unique", async () => {
      ...
      await assert.rejects(
        sql`INSERT INTO blog_tags (id, name, slug) VALUES ('duplicate-tag', 'pdf', 'other-pdf')`,
        { code: "23505" },
      );
      await sql`INSERT INTO blog_published_post_tags (post_id, tag_id) VALUES ('post-b', 'tag')`;
      const [usage] =
        await sql`SELECT COUNT(*)::integer AS count FROM blog_published_post_tags WHERE tag_id = 'tag'`;
      assert.equal(usage.count, 2);
    });

    await context.test(
      "schedules freeze a same-post revision and permit only one active request",
      async () => {
        await assert.rejects(
        ...
        const [schedule] =
          await sql`SELECT revision_id FROM blog_post_schedules WHERE post_id = 'post-a'`;
    ...
    await context.test(
      ...
      async () => {
        ...
        const [post] =
          await sql`SELECT created_by, draft_updated_by FROM blog_posts WHERE id = 'post-a'`;
        assert.deepEqual(post, { created_by: null, draft_updated_by: null });
        const [revision] =
          await sql`SELECT created_by, document FROM blog_revisions WHERE id = 'revision-a'`;
