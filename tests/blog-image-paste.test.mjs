...
const Image = Node.create({ name: 'image', group: 'block', atom: true, addAttributes: () => ({ publicId: { default: '' }, alt: { default: '' } }) });
const file = (name, type = 'image/png') => new File(['image bytes'], name, { type });
const paragraph = text => ({ type: 'paragraph', content: [{ type: 'text', text }] });
const uploaded = name => ({ publicId: name, version: 1, format: 'png', width: 100, height: 100, alt: '', caption: '' });
function editorFor() {
  const editor = new Editor({ element: null, extensions: [StarterKit, Image], content: { type: 'doc', content: [paragraph('Before'), paragraph('After')] } });
...
function clipboard(files, items = files.map(image => ({ kind: 'file', type: image.type, getAsFile: () => image }))) {
  return { clipboardData: { files, items }, defaultPrevented: false, preventDefault() { this.defaultPrevented = true; } };
}
...
test('clipboard previews appear immediately, map through edits, and atomically become ordered images', async t => {
  ...
  const urls = [];
  const revoked = [];
  t.mock.method(URL, 'createObjectURL', image => { const url = `blob:preview-${urls.length}`; urls.push([image.name, url]); return url; });
  ...
  assert.deepEqual(urls.map(([name]) => name), ['one.png', 'two.png']);
  ...
  assert.deepEqual(editor.getJSON().content.map(node => node.type), ['paragraph', 'image', 'image', 'paragraph']);
...
test('file-only clipboard works while text, missing data, and unrelated files retain normal paste', async () => {
  const editor = editorFor();
  let uploads = 0;
  const upload = async image => { uploads++; return uploaded(image.name); };
  for (const event of [clipboard([]), clipboard([file('notes.txt', 'text/plain')]), { clipboardData: null }, clipboard([], [{ kind: 'string', type: 'text/plain' }])]) {
...
test('failed uploads remove previews and leave the original selection intact while preserving the upload error', async t => {
  ...
  const errors = [];
  const requests = [];
  pasteBlogImages(editor, clipboard([file('first.png'), file('bad.gif', 'image/gif'), file('last.png')]), async image => {
    requests.push(image.name);
...
test('unavailable editors remove pending previews immediately and never write after disposal', async t => {
  ...
  for (const stop of ['read-only', 'destroyed']) {
    ...
    const pending = new Promise(resolve => { finish = resolve; });
    const errors = [];
    pasteBlogImages(editor, clipboard([file('pending.png'), file('next.png')]), async () => pending, error => errors.push(error));
    ...
    Object.defineProperty(editor, 'isDestroyed', { configurable: true, value: stop === 'destroyed' });
    finish(uploaded('pending.png'));
    await setImmediate();
    assert.deepEqual(editor.getJSON(), before);
...
test('silent read-only changes remove decorations and dispose preview resources through the plugin view', async t => {
  ...
  const pending = new Promise(resolve => { finish = resolve; });
  const errors = [];
  pasteBlogImages(editor, clipboard([file('pending.png')]), () => pending, error => errors.push(error));
  const preview = editor.state.plugins.find(plugin => !existing.has(plugin));
...
test('finishing one paste leaves a separate pending paste preview alive', async () => {
  ...
  await setImmediate();
  assert.deepEqual(editor.getJSON().content.filter(node => node.type === 'image').map(node => node.attrs.publicId), ['first.png', 'second.png']);
  assert.equal(editor.state.plugins.some(plugin => previews.includes(plugin)), false);
  editor.destroy();
});
...
test('unexpected upload errors produce recoverable feedback without unhandled rejection', async () => {
  const editor = editorFor();
  const errors = [];
  pasteBlogImages(editor, clipboard([file('failed.png')]), async () => { throw new Error('network'); }, error => errors.push(error));
  await setImmediate();
  assert.equal(errors.length, 1);
  assert.match(errors[0], /paste.*again/i);
  assert.equal(editor.state.doc.childCount, 2);
...
test('insertion errors still report recovery and remove previews after the upload succeeds', async t => {
  ...
  const revoked = [];
  t.mock.method(URL, 'revokeObjectURL', url => revoked.push(url));
  t.mock.method(editor, 'commands', () => ({ insertContentAt() { throw new Error('insertion unavailable'); } }), { getter: true });
  const errors = [];
  pasteBlogImages(editor, clipboard([file('pending.png')]), async () => uploaded('pending.png'), error => errors.push(error));
  await setImmediate();
  assert.equal(errors.length, 1);
  assert.match(errors[0], /paste.*again/i);
  assert.deepEqual(editor.state.plugins, existing);
