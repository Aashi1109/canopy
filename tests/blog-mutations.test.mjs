...
import {
  db, auditEventsTable, blogPostsTable as posts, blogRevisionsTable as revisions,
  blogPostSchedulesTable as schedules, blogCategoriesTable as categories,
  blogTagsTable as tags, blogPublishedPostTagsTable as publishedTags,
} from '../packages/database/src/index.ts';
import {
  createBlogPost, duplicateBlogPost, saveBlogDraft, publishBlogPost, scheduleBlogPost, retryBlogSchedule,
  cancelBlogSchedule, unpublishBlogPost, trashBlogPost, restoreTrashedBlogPost,
  restoreBlogRevision, saveBlogTerm, publishDueBlogPosts,
...
function document(title = 'Original article') {
  return validateBlogDocument({
    ...createBlogDocument(title), excerpt: 'A useful article.',
    body: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'Useful content.' }] }] },
    category: { id: category.id, label: category.name }, tags: [{ id: tag.id, label: tag.name }],
  });
}
function post(overrides = {}) {
  const draft = document();
  return {
    id: 'post-1', slug: 'original-article', draftDocument: draft, draftHash: blogDocumentHash(draft),
    version: 5, revisionSequence: 2, lastCheckpointAt: new Date(NOW.getTime() - 120_000),
    draftUpdatedAt: new Date(NOW.getTime() - 120_000), draftUpdatedBy: 'actor',
    publishedRevisionId: 'revision-live', publishedCategoryId: category.id,
    firstPublishedAt: new Date('2026-09-01T10:00:00Z'), publishedUpdatedAt: new Date('2026-09-01T10:00:00Z'),
    createdBy: 'actor', createdAt: new Date('2026-08-01T10:00:00Z'), updatedAt: NOW,
    trashedAt: null, ...overrides,
  };
}
function revision(row, overrides = {}) {
  return {
    id: 'revision-2', postId: row.id, revisionNumber: row.revisionSequence,
    document: row.draftDocument, contentHash: row.draftHash, reason: 'manual_save',
    createdBy: 'actor', createdAt: row.lastCheckpointAt, sourceRevisionId: null, ...overrides,
  };
}
function permission({ missing, status = 'active' } = {}) {
  const access = structuredClone(ADMIN_ACCESS);
  if (missing) delete access.blog[missing];
  return [{ status, roleId: 'blog-manager', roleName: 'Blog manager', roleDescription: 'Custom blog role', roleAccess: access, roleIsSystem: false }];
}
...
async function withDatabase(reads, callback, options = {}) {
  const original = { transaction: db.transaction, select: db.select, update: db.update, execute: db.execute };
  const queue = [...reads];
  const state = { committed: [], rolledBack: [], post: structuredClone(options.post ?? post()), readQueries: [], attempts: 0 };
  let staged;
  let conflicts = options.insertConflicts ?? 0;
  const select = () => {
    const entry = {};
    const chain = {
      from(table) { entry.table = table; return chain; },
      where(condition) { entry.condition = query(condition); return chain; },
      innerJoin: () => chain, leftJoin: () => chain, limit: () => chain,
      for: () => chain, orderBy: () => chain,
      then(resolve, reject) {
  ...
  const mutation = (kind, table) => {
    ...
    const perform = () => {
      ...
      if (kind === 'insert') {
        ...
        if (table === posts) rows = rows.map(value => ({
          ...post({ version: 1, revisionSequence: 0, lastCheckpointAt: null,
            publishedRevisionId: null, publishedCategoryId: null, firstPublishedAt: null, publishedUpdatedAt: null }), ...value,
        }));
      } else if (kind === 'update') {
    ...
    const chain = {
      values(values) { entry.values = values; return chain; },
      set(values) { entry.values = values; return chain; },
      where(condition) { entry.condition = query(condition); return chain; },
  ...
  const tx = { select, insert: table => mutation('insert', table), update: table => mutation('update', table),
    delete: table => mutation('delete', table), execute };
  db.select = select;
  db.update = tx.update;
  db.execute = execute;
...
const writes = (state, table, kind) => state.committed.filter(entry => entry.table === table && (!kind || entry.kind === kind));
...
test('every administrative blog mutation rejects missing permission before content writes', async () => {
  ...
  for (const [missing, invoke] of cases) await withDatabase([permission({ missing })], async state => {
...
test('stale versions, trash, and absent articles reject saves without overwriting work', async () => {
  for (const [rows, code] of [[[post({ version: 6 })], 'CONFLICT'], [[post({ trashedAt: NOW })], 'VALIDATION'], [[], 'NOT_FOUND']]) {
    await withDatabase([permission(), rows], async state => {
      await assert.rejects(() => saveBlogDraft('actor', { postId: 'post-1', version: 5, document: document() }), { code });
      assert.deepEqual(state.committed, []);
    });
  }
});
...
test('autosave canonicalizes taxonomy, checkpoints changed work, and leaves public state and slug intact', async () => {
  ...
  await withDatabase([permission(), [current], [category], [tag], clock(), [revision(current)]], async state => {
    const result = await saveBlogDraft('actor', { postId: current.id, version: current.version, document: changed });
...
test('autosaves persist before checkpoint interval and unchanged saves do not churn versions', async () => {
  ...
  for (const changed of [true, false]) await withDatabase([permission(), [current], [category], [tag], clock()], async state => {
    const result = await saveBlogDraft('actor', { postId: current.id, version: current.version,
...
test('manual save captures uncheckpointed work and reuses an identical latest revision', async () => {
  const current = post({ lastCheckpointAt: new Date(NOW.getTime() - 1000) });
  for (const sameRevision of [true, false]) await withDatabase([
    permission(), [current], [category], [tag], clock(),
    [revision(current, sameRevision ? {} : { contentHash: 'older-content' })],
  ], async state => {
    const result = await saveBlogDraft('actor', { postId: current.id, version: current.version, document: current.draftDocument, mode: 'manual' });
...
test('revision restoration backs up unsaved-to-history content and changes only the working draft', async () => {
  ...
  const source = revision(current, { id: 'revision-1', revisionNumber: 1, document: historical, contentHash: blogDocumentHash(historical) });
  await withDatabase([permission(), [current], [source], [category], [tag], clock(),
    [revision(current, { contentHash: 'prior-checkpoint' })], []], async state => {
    const result = await restoreBlogRevision('actor', { postId: current.id, version: current.version, revisionId: source.id });
    ...
    assert.deepEqual(snapshots.map(row => row.reason), ['restore_backup', 'restore']);
    assert.deepEqual(snapshots.map(row => row.revisionNumber), [3, 4]);
...
test('create retries unique slug conflicts with a random suffix and records an initial revision', async () => {
  await withDatabase([permission(), clock(), []], async state => {
  ...
  await withDatabase([permission()], async state => {
    await assert.rejects(() => createBlogPost('actor', { title: 'Collisions' }), { code: 'CONFLICT' });
...
test('unknown taxonomy references and incomplete article content fail before snapshot or publication writes', async () => {
  ...
  await withDatabase([permission(), [current], []], async state => {
    await assert.rejects(() => saveBlogDraft('actor', { postId: current.id, version: current.version, document: current.draftDocument }), /existing category/);
    assert.equal(state.committed.length, 0);
  });
  const incomplete = post({ draftDocument: { ...document(), excerpt: '' } });
  await withDatabase([permission(), [incomplete], [category], [tag]], async state => {
    await assert.rejects(() => publishBlogPost('actor', { postId: current.id, version: current.version }), /Excerpt is required/);
    assert.equal(state.committed.length, 0);
  });
});
...
test('publishing promotes a snapshot and clears schedule, with audit failure rolling everything back', async () => {
  ...
  for (const failAudit of [false, true]) await withDatabase(reads(), async state => {
...
test('scheduling freezes the saved revision and rejects due or past times', async () => {
  const current = post();
  const snapshot = revision(current);
  await withDatabase([permission(), [current], clock(), [category], [tag], [snapshot]], async state => {
    const result = await scheduleBlogPost('actor', { postId: current.id, version: current.version, scheduledAt: '2026-09-16T11:12:00Z' });
  ...
  await withDatabase([permission(), [current], clock()], async state => {
    await assert.rejects(() => scheduleBlogPost('actor', { postId: current.id, version: current.version, scheduledAt: NOW.toISOString() }), /future publication time/);
    assert.equal(state.committed.length, 0);
  });
});

test('live article trash requires publication authority as well as archive permission', async () => {
  await withDatabase([permission({ missing: 'publish' }), [post()], permission({ missing: 'publish' })], async state => {
    await assert.rejects(() => trashBlogPost('actor', { postId: 'post-1', version: 5 }), /Missing permission: blog.publish/);
    assert.equal(state.committed.length, 0);
  });
});
...
test('unpublishing removes live projections and schedule while trash restoration stays a draft', async () => {
  const current = post();
  await withDatabase([permission(), [current], clock()], async state => {
    const result = await unpublishBlogPost('actor', { postId: current.id, version: current.version });
    assert.equal(result.publishedRevisionId, null);
  ...
  await withDatabase([permission(), [trashed], clock()], async state => {
    const result = await restoreTrashedBlogPost('actor', { postId: trashed.id, version: trashed.version });
...
test('taxonomy records track creating and editing administrators without changing their slug', async () => {
  for (const kind of ['category', 'tag']) {
    ...
    await withDatabase([permission(), [existing], [], clock()], async state => {
...
test('taxonomy creation and rename reject names that could corrupt saved article labels', async () => {
  for (const name of ['Bad\u0001name', 'Invalid\ud800name']) {
    for (const id of [undefined, category.id]) await withDatabase([], async state => {
...
test('scheduler publishes the frozen revision, preserving newer draft and initial publication date', async () => {
  ...
  const frozen = revision(current, { id: 'frozen', document: document('Scheduled content'), contentHash: blogDocumentHash(document('Scheduled content')) });
  const scheduled = { id: 'schedule-1', postId: current.id, revisionId: frozen.id, scheduledAt: new Date(NOW.getTime() - 1000), scheduledBy: 'actor' };
  await withDatabase([clock(), [scheduled], permission(), [current], [scheduled], [frozen], clock(), [category], [tag], [{ count: 0 }]], async state => {
    assert.deepEqual(await publishDueBlogPosts(), { attempted: 1, published: 1, failed: 0, remaining: 0 });
...
test('retry-now publishes only the frozen due revision and records the invoking administrator', async () => {
  const current = post({ draftDocument: document('Newer unscheduled draft') });
  current.draftHash = blogDocumentHash(current.draftDocument);
  const frozen = revision(current, { id: 'frozen', document: document('Scheduled version'), contentHash: blogDocumentHash(document('Scheduled version')) });
  const scheduled = { id: 'schedule-1', postId: current.id, revisionId: frozen.id, scheduledAt: NOW,
    scheduledBy: 'original-publisher', lastErrorCode: 'TEMPORARY_FAILURE' };
  await withDatabase([permission(), [scheduled], permission(), [current], [scheduled], clock(), [frozen], [category], [tag]], async state => {
    const result = await retryBlogSchedule('retrying-admin', { postId: current.id, version: current.version });
...
test('retry-now requires an active original publisher; deleted, suspended and revoked publishers must reschedule', async () => {
  const scheduled = { id: 'schedule-1', postId: 'post-1', revisionId: 'frozen', scheduledAt: NOW, scheduledBy: 'original-publisher' };
  for (const originalPermission of [permission({ missing: 'publish' }), permission({ status: 'suspended' }), []]) {
    await withDatabase([permission(), [scheduled], originalPermission], async state => {
      await assert.rejects(() => retryBlogSchedule('retrying-admin', { postId: 'post-1', version: 5 }), /Access denied|Missing permission/);
      assert.equal(state.committed.length, 0);
    });
  }
  await withDatabase([permission(), [{ ...scheduled, scheduledBy: null }]], async state => {
    await assert.rejects(() => retryBlogSchedule('retrying-admin', { postId: 'post-1', version: 5 }), /Scheduling account/);
    assert.equal(state.committed.length, 0);
  });
});
...
test('retry-now rejects missing, replaced, future and invalid-version schedules without publication', async () => {
  const current = post();
  const scheduled = { id: 'schedule-1', postId: current.id, revisionId: 'frozen', scheduledAt: NOW, scheduledBy: 'actor' };
  const cases = [
    ...
    [[permission(), [scheduled], [current], [{ ...scheduled, scheduledAt: new Date(NOW.getTime() + 1) }], clock()], 'VALIDATION'],
    [[permission(), [scheduled], [current], [scheduled], clock(), []], 'NOT_FOUND'],
  ];
  for (const [reads, code] of cases) await withDatabase(reads, async state => {
    await assert.rejects(() => retryBlogSchedule('actor', { postId: current.id, version: current.version }), { code });
    assert.equal(state.committed.length, 0);
  });
});
...
test('failed retry publication rolls back schedule removal and live changes when audit cannot be saved', async () => {
  ...
  const scheduled = { id: 'schedule-1', postId: current.id, revisionId: frozen.id, scheduledAt: NOW, scheduledBy: 'actor' };
  await withDatabase([permission(), [scheduled], [current], [scheduled], clock(), [frozen], [category], [tag]], async state => {
    await assert.rejects(() => retryBlogSchedule('actor', { postId: current.id, version: current.version }), /simulated database write failure/);
    assert.equal(state.committed.length, 0);
    assert.equal(state.post.publishedRevisionId, current.publishedRevisionId);
...
test('scheduler leaves a cancelled or replaced request alone after re-reading it under lock', async () => {
  const current = post();
  const stale = { id: 'old-schedule', postId: current.id, revisionId: 'revision-2', scheduledAt: NOW, scheduledBy: 'actor' };
  await withDatabase([clock(), [stale], permission(), [current], [], [{ count: 1 }]], async state => {
    assert.deepEqual(await publishDueBlogPosts(), { attempted: 1, published: 0, failed: 0, remaining: 1 });
    assert.equal(state.committed.length, 0);
    const reread = state.readQueries.find(entry => entry.table === schedules && entry.condition?.params.includes(stale.id));
...
test('revoked scheduling permissions preserve due work and scope failure diagnostics to request identity', async () => {
  const scheduled = { id: 'old-schedule', postId: 'post-1', revisionId: 'revision-2', scheduledAt: NOW, scheduledBy: 'actor' };
  await withDatabase([clock(), [scheduled], permission({ missing: 'publish' }), [{ count: 1 }]], async state => {
    assert.deepEqual(await publishDueBlogPosts(), { attempted: 1, published: 0, failed: 1, remaining: 1 });
...
test('scheduler continues with other posts when diagnostic persistence also fails', async (t) => {
  ...
  const first = { id: 'schedule-1', postId: 'post-1', revisionId: 'revision-2', scheduledAt: NOW, scheduledBy: null };
  const second = { ...first, id: 'schedule-2', postId: 'post-2' };
  await withDatabase([clock(), [first, second], [{ count: 2 }]], async state => {
    assert.deepEqual(await publishDueBlogPosts(), { attempted: 2, published: 0, failed: 2, remaining: 2 });
    assert.equal(state.attempts, 2);
    assert.equal(warnings.length, 2);
    assert.deepEqual(warnings[0][1], { postId: first.postId, scheduleId: first.id, code: 'PUBLISHER_FORBIDDEN' });
...
test('scheduler does not reattempt the same post when rescheduling appears on a later batch', async () => {
  const firstBatch = Array.from({ length: 50 }, (_, index) => ({
    id: `schedule-${index}`, postId: `post-${index}`, revisionId: `revision-${index}`,
    scheduledAt: NOW, cursorTime: '2026-09-16T10:30:00.000123Z', scheduledBy: null,
  }));
  ...
  await withDatabase([clock(), firstBatch, [replacement], [{ count: 50 }]], async state => {
    assert.deepEqual(await publishDueBlogPosts(), { attempted: 50, published: 0, failed: 50, remaining: 50 });
    ...
    assert.ok(laterBatch.condition.params.includes(firstBatch.at(-1).cursorTime), 'cursor retains PostgreSQL microseconds');
  });
});
