...
const hooks = registerHooks({
  resolve(specifier, context, nextResolve) {
    if (context.parentURL === sourceUrl) {
      ...
      if (specifier === "../admin/adminMutations.ts")
        return {
          shortCircuit: true,
          url: moduleUrl(`
        const fixture = globalThis.__blogImageTest;
...
test("uploads immutable versioned assets with permission checks and audit attribution", async () => {
  ...
  assert.deepEqual(
    { ...first, publicId: "id" },
    { publicId: "id", version: 1234, format: "png", width: 1, height: 1, alt: "", caption: "" },
  );
...
test("revoked permissions and failed audit do not return a successful asset", async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error.code === "UPLOAD_FINALIZATION_FAILED" &&
      /recorded/i.test(error.message) &&
      !error.message.includes("Audit unavailable"),
  );
});
...
test("file size, MIME type and raster signature are validated before network upload", async () => {
  for (const input of [
    ...
    new File(["<svg></svg>"], "bad.png", { type: "image/png" }),
    new File([png], "bad.jpg", { type: "image/jpeg" }),
    new File([png], "bad.svg", { type: "image/svg+xml" }),
    new File(["RIFF1234WEBP"], "bad.webp", { type: "image/webp" }),
  ]) {
...
test("recognizes JPEG and WebP signatures and permits the exact byte limit", async () => {
  ...
  const boundaryPng = Buffer.alloc(5 * 1024 * 1024);
  png.copy(boundaryPng);
  for (const [bytes, type] of [
    [Buffer.from([0xff, 0xd8, 0xff, 0xe0]), "image/jpeg"],
    [webp, "image/webp"],
    [boundaryPng, "image/png"],
  ]) {
...
test("provider failures do not expose credentials and invalid provider metadata is rejected", async () => {
  ...
  for (const response of [
    ...
    await assert.rejects(
      () => uploadBlogImage("admin", file()),
      (error) => error.code === "UPLOAD_INVALID_RESPONSE" && /retry/i.test(error.message),
    );
  }
});
...
test("database outages stay actionable while permission revocation remains forbidden", async () => {
  ...
  await assert.rejects(
    () => uploadBlogImage("admin", file()),
    (error) => error.code === "UPLOAD_FINALIZATION_FAILED" && !error.message.includes("password"),
  );
  assert.equal(fixture.uploads.length, 1);
...
test("known upload transport failures expose only safe diagnostic codes with recovery guidance", async () => {
  for (const [providerError, diagnostic, guidance] of [
    ...
    fixture.uploadError = {
      ...providerError,
      message: "credential-secret-private-provider-detail",
  ...
  fixture.uploadError = {
    code: "private-secret-unrecognized-code",
    http_code: "private-secret",
