...
import { BlogTableCell, BlogTableHeader, appendBlogTableAxis, blogTableChangeFits, deleteBlogTableAxis, duplicateBlogTableAxis, formatBlogTableCells, getBlogTableContext, getBlogTableDragDelta, getBlogTableStateAtCell, moveBlogTableAxis, resizeBlogTableAxis, selectBlogTableAxis, setBlogTableColumnWidth } from '../app/admin/(protected)/blog/lib/tableEditing.ts';

const cell = (text, attrs = {}) => ({ type: 'tableCell', attrs, content: [{ type: 'paragraph', content: [{ type: 'text', text }] }] });
const row = (...content) => ({ type: 'tableRow', content });
function createEditor(rows = [row(cell('A'), cell('B')), row(cell('C'), cell('D'))]) {
  return new Editor({ element: null, extensions: [StarterKit, TableKit.configure({ tableCell: false, tableHeader: false }), BlogTableCell, BlogTableHeader], content: { type: 'doc', content: [{ type: 'table', content: rows }, { type: 'paragraph' }] } });
}
...
const cells = editor => editor.getJSON().content[0].content.map(row => row.content.map(cell => cell.content.map(block => block.content?.map(text => text.text).join('') ?? '').join('')));

test('hover resolution preserves the live selection and edge actions target the hovered second table', () => {
  for (const axis of ['row', 'column']) {
    const editor = createEditor();
    editor.commands.insertContentAt(editor.state.doc.content.size, { type: 'table', content: [row(cell('Second A'), cell('Second B'))] });
    select(editor, 0, 0);
    ...
    assert.equal(appendBlogTableAxis(axis)(target, transaction => editor.view.dispatch(transaction)), true);
...
test('hover targets reject invalid or removed cells without changing editor content or selection', () => {
  ...
  for (const invalid of [-1, 0, 1, position + 1, 1.5, NaN, editor.state.doc.content.size, editor.state.doc.content.size + 100]) {
...
test('table edge actions append after the last row or column regardless of cursor position', () => {
  for (const axis of ['row', 'column']) {
    ...
    assert.deepEqual(cells(editor), axis === 'row' ? [['A', 'B'], ['C', 'D'], ['', '']] : [['A', 'B', ''], ['C', 'D', '']]);
...
test('duplicate row and column preserve cell attributes and select the copy', () => {
  for (const axis of ['row', 'column']) {
    const editor = createEditor([row(cell('A', { colwidth: [180], align: 'right', backgroundColor: '#dbeafe' }), cell('B')), row(cell('C', { colwidth: [180] }), cell('D'))]);
    select(editor, 0, 0);
    assert.equal(run(editor, duplicateBlogTableAxis(axis)), true);
    assert.deepEqual(cells(editor), axis === 'row' ? [['A', 'B'], ['A', 'B'], ['C', 'D']] : [['A', 'A', 'B'], ['C', 'C', 'D']]);
...
test('keyboard column width updates every intersecting cell including a merged cell', () => {
  const editor = createEditor([row(cell('Merged', { colspan: 2, colwidth: [120, 140] })), row(cell('C', { colwidth: [120] }), cell('D', { colwidth: [140] }))]);
  select(editor, 1, 1);
...
test('table commands leave paragraphs untouched outside a table', () => {
  ...
  const before = editor.getJSON();
  for (const command of [appendBlogTableAxis('row'), appendBlogTableAxis('column'), selectBlogTableAxis('row'), moveBlogTableAxis('column', 1), duplicateBlogTableAxis('row'), formatBlogTableCells({ align: 'right' }), setBlogTableColumnWidth(200)]) assert.equal(run(editor, command), false);
  assert.deepEqual(editor.getJSON(), before);
  editor.destroy();
});
...
test('native movement keeps merged spans valid and never duplicates merged content', () => {
  const editor = createEditor([row(cell('Merged', { colspan: 2 })), row(cell('C'), cell('D')), row(cell('E'), cell('F'))]);
  select(editor, 0, 0);
  assert.equal(run(editor, moveBlogTableAxis('row', 1)), true);
...
test('table changes cannot exceed persisted column or article node limits', () => {
  ...
  editor.commands.insertContentAt(editor.state.doc.content.size, Array.from({ length: 10000 }, () => ({ type: 'paragraph' })));
...
test('drag resizing appends and removes trailing rows or columns with one atomic dispatch', () => {
  for (const axis of ['row', 'column']) {
    ...
    select(editor, 0, 0);
    let calls = 0;
    const dispatch = transaction => { calls++; editor.view.dispatch(transaction); };
...
test('drag resizing preserves native header behavior and targets only the hovered table', () => {
  const editor = createEditor();
  const firstTable = editor.state.doc.firstChild;
  editor.commands.insertContentAt(editor.state.doc.content.size, { type: 'table', content: [row({ ...cell('Header'), type: 'tableHeader' })] });
  select(editor, 0, 0);
  ...
  assert.equal(resizeBlogTableAxis('row', 2)(target, transaction => editor.view.dispatch(transaction)), true);
  assert.ok(editor.state.doc.firstChild.eq(firstTable));
...
test('invalid, cancelled, dry-run, and oversized drags do not partially mutate a table', () => {
  ...
  for (const delta of [NaN, Infinity, -Infinity, 0.5, 1001, -1001, -2]) assert.equal(resizeBlogTableAxis('row', delta)(editor.state, dispatch), false);
  ...
  editor.commands.insertContentAt(editor.state.doc.content.size, Array.from({ length: 9977 }, () => ({ type: 'paragraph' })));
  select(editor, 0, 0);
...
test('each completed resize is one undo event that restores removed content', () => {
  ...
  let state = EditorState.create({ schema: editor.schema, doc: original, selection: editor.state.selection, plugins: [history()] });
...
test('typing immediately after a resize stays a separate undo event', () => {
  ...
  let state = EditorState.create({ schema: editor.schema, doc: original, selection: editor.state.selection, plugins: [history()] });
  let resizeTime = 0;
  assert.equal(resizeBlogTableAxis('row', 2)(state, transaction => { resizeTime = transaction.time; state = state.apply(transaction); }), true);
  const expanded = state.doc;
...
test('inward drag follows unequal column widths and row heights instead of a uniform guess', () => {
  ...
  for (const [distance, expected] of [[-49, 0], [-50, -1], [-269, -1], [-270, -2], [-500, -2]]) assert.equal(getBlogTableDragDelta({ ...rows, distance }), expected);
});
...
test('drag delta bounds preserve direction and refuse destructive guesses without exact edges', () => {
  ...
  for (const edges of [[], [0, 30], [0, NaN, 90], [0, 90, 30], [0, 30, 30], [0, 30, 120]]) assert.equal(getBlogTableDragDelta({ ...input, edges }), 0);
  ...
  assert.equal(getBlogTableDragDelta({ ...input, count: 1200, edges: Array.from({ length: 1200 }, (_, index) => index), end: 1200, distance: -1200 }), -1000);
});
