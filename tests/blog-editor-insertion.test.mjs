...
const Image = Node.create({ name: 'image', group: 'block', atom: true, addAttributes: () => ({ publicId: { default: '' }, version: { default: 1 }, alt: { default: '' }, caption: { default: '' } }) });
...
function editorFor(content) {
  const editor = new Editor({ element: null, extensions: [StarterKit, Image, TableKit], content: { type: 'doc', content } });
...
test('append preserves a selected last image when adding another image, block, or table', () => {
  for (const kind of ['image', 'blockquote', 'table']) {
    ...
    const content = kind === 'image' ? image('new') : kind === 'table' ? createTable(editor.schema, 3, 3, true).toJSON() : { type: 'blockquote', content: [paragraph('quote')] };
...
test('one captured upload inserts multiple images together in clipboard order', () => {
  ...
  assert.deepEqual(editor.getJSON().content.map(node => node.attrs?.publicId || node.content?.[0]?.text), ['Before', 'first', 'second', 'After']);
...
test('table dimensions preserve chosen rows, columns, header row, and the existing image', () => {
  for (const [columns, rows] of [[1, 1], [5, 2], [2, 5], [20, 20]]) {
    const editor = editorFor([image('original')]);
