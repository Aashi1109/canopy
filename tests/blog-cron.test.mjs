...
const request = (authorization, method = "POST") => new Request(`https://smarttools.test${path}`, {
  method,
...
test("publishing authenticates before invoking domain logic and returns only safe counts", async () => {
  let calls = 0;
  const publish = async () => { calls++; return { ...counts, privateError: "database-password" }; };
  for (const authorization of [undefined, "Basic example", "Bearer wrong", `Bearer ${secret}, extra`, `Bearer ${secret} extra`]) {
...
test("GET cannot publish and failed domain requests never expose internal error details", async () => {
  let calls = 0;
  const publish = async () => { calls++; throw new Error("postgres://private-password@database"); };
...
test("scheduled handler uses its service binding and awaits the authenticated POST result", async () => {
  ...
  const env = {
    ...
    WORKER_SELF_REFERENCE: { async fetch(req) { received = req; return Response.json(counts); } },
  };
...
test("remote target is explicit, uses HTTPS, and cannot forward credentials through redirects", async () => {
  ...
  const env = {
    ...
    WORKER_SELF_REFERENCE: { fetch() { bindingCalls++; throw new Error("should not use binding"); } },
  };
  const fetchRemote = async (req) => { received = req; return Response.json(counts); };
  assert.deepEqual(await runBlogPublishCron(env, fetchRemote), counts);
  assert.equal(received.url, env.BLOG_PUBLISH_URL);
  assert.equal(received.redirect, "manual");
  assert.equal(bindingCalls, 0);
  await assert.rejects(runBlogPublishCron(env, async () => new Response(null, {
    status: 307, headers: { location: "https://untrusted.example.test" },
  })), /HTTP 307/);
});

test("invalid cron configuration fails before sending any secret", async () => {
  let calls = 0;
  const fetchRemote = async () => { calls++; return Response.json(counts); };
  for (const target of [
    `http://docker.example.test${path}`, `https://user:pass@docker.example.test${path}`,
    `https://docker.example.test${path}?next=https://untrusted.test`,
    `https://docker.example.test${path}#fragment`, "https://docker.example.test/wrong-path", "invalid",
  ]) {
    await assert.rejects(runBlogPublishCron({ BLOG_SCHEDULER_SECRET: secret, BLOG_PUBLISH_URL: target }, fetchRemote), /configuration/);
  }
  await assert.rejects(runBlogPublishCron({ BLOG_PUBLISH_URL: `https://docker.example.test${path}` }, fetchRemote), /configuration/);
...
test("cron consumes successful responses and rejects unsafe, oversized or failed outcomes", async () => {
  const env = { BLOG_SCHEDULER_SECRET: secret, BLOG_PUBLISH_URL: `https://docker.example.test${path}` };
  ...
  for (const value of [null, {}, { ...counts, published: -1 }, { ...counts, remaining: 1.5 }]) {
    await assert.rejects(runBlogPublishCron(env, async () => Response.json(value)), /invalid response/);
  }
  await assert.rejects(runBlogPublishCron(env, async () => new Response("x".repeat(4097))), /invalid response/);
  await assert.rejects(runBlogPublishCron(env, async () => new Response("private database error", { status: 503 })), /HTTP 503/);
  await assert.rejects(runBlogPublishCron(env, async () => { throw new Error(`Secret ${secret}`); }), {
...
test("actual Worker schedule re-enters the fetch database wrapper through its self binding", async (t) => {
  ...
  try {
    ...
    const env = {
      ...
      WORKER_SELF_REFERENCE: { fetch: (req) => worker.fetch(req, env, { waitUntil: (task) => waits.push(task) }) },
    };
