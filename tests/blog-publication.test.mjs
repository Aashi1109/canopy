...
const post = { slug:'first-post', document:{...createBlogDocument('Title <&>'),excerpt:'Useful summary',seoTitle:'SEO title'}, firstPublishedAt:new Date('2026-09-16T00:00:00Z'), publishedUpdatedAt:new Date('2026-09-16T01:00:00Z') };
test('article metadata uses canonical immutable slug, explicit overrides and publication dates', () => {
  ...
  try {
    const imagePost = {...post, document:{...post.document,coverImage:{publicId:'smarttools/blog/aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee',version:123,format:'png',width:1200,height:630,alt:'Cover',caption:''}}};
    const withImage = blogArticleMetadata(imagePost,'https://example.test');
    assert.equal(withImage.twitter.card,'summary_large_image');
    assert.equal(withImage.openGraph.images[0].url,'https://res.cloudinary.com/test-cloud/image/upload/v123/smarttools/blog/aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee.png');
    assert.equal(JSON.parse(blogStructuredData(imagePost,'https://example.test')).image,withImage.openGraph.images[0].url);
  } finally {
...
test('structured data is safe to embed in an application/ld+json script', () => {
  const json=blogStructuredData({...post,document:{...post.document,title:'</script><script>alert(1)</script>'}},'https://example.test');
...
test('RSS escapes stored text and uses publication dates rather than draft updates', () => {
  const xml=buildBlogFeed([{slug:post.slug,title:'<Title & "quote">',excerpt:'Description </item>',authorName:'Writer',firstPublishedAt:post.firstPublishedAt,publishedUpdatedAt:post.publishedUpdatedAt}],'https://example.test');
