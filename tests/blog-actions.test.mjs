...
const names = ['createBlogPost','duplicateBlogPost','saveBlogDraft','publishBlogPost','scheduleBlogPost','cancelBlogSchedule','retryBlogSchedule','unpublishBlogPost','trashBlogPost','restoreTrashedBlogPost','restoreBlogRevision','saveBlogTerm'];
const reads = ['getBlogPost','getBlogRevision','listBlogPosts','listBlogRevisions','listBlogTaxonomy'];
const state = { actor: 'session-admin', calls: [], error: null, data: { id: 'post-id' }, sessionError: null, BlogError, BlogImageUploadError };
globalThis.__blogActionTest = state;
const stub = (source) => ({ shortCircuit: true, url: `data:text/javascript,${encodeURIComponent(source)}` });
const functions = (exports) => exports.map(name => `export const ${name} = async (...args) => {const s=globalThis.__blogActionTest;s.calls.push({name:${JSON.stringify(name)},args});if(s.error)throw s.error;return s.data;};`).join('\n');
const hooks = registerHooks({
  resolve(specifier, context, next) {
    if (context.parentURL === url) {
      if (specifier.endsWith('/admin/access')) return stub('export async function getActorUserId(){const s=globalThis.__blogActionTest;if(s.sessionError)throw s.sessionError;return s.actor;}');
      if (specifier.endsWith('/blog/mutations')) return stub(`export const BlogError=globalThis.__blogActionTest.BlogError;${functions(names)}`);
      if (specifier.endsWith('/blog/queries')) return stub(functions(reads));
      if (specifier.endsWith('/blog/images')) return stub(`export const BlogImageUploadError=globalThis.__blogActionTest.BlogImageUploadError;${functions(['uploadBlogImage'])}`);
...
function reset() { state.calls=[]; state.error=null; state.sessionError=null; state.data={id:'post-id'}; }
test.after(() => { hooks.deregister(); delete globalThis.__blogActionTest; });
...
test('known failures are actionable while unexpected database/provider details stay private', async () => {
  reset();
  for (const [error, code] of [[new BlogError('CONFLICT','Reload latest version'),'CONFLICT'],[new BlogValidationError('Excerpt is required'),'VALIDATION'],[new AuthorizationError('Private role data'),'FORBIDDEN'],[new Error('postgres://secret@example.invalid password=hidden'),'TEMPORARY_FAILURE']]) {
    state.error=error;
...
test('private preview renders validated content and keeps revision reads bound to post and actor', async () => {
  ...
  document.body={type:'doc',content:[{type:'paragraph',content:[{type:'text',text:'<script>secret draft</script>'}]}]};
  state.data={document};
  const result=await actions.readBlogAction({operation:'preview',postId:'post',revisionId:'rev'});
...
test('read and upload envelopes reject forged actor fields and extra multipart values', async () => {
  reset();
  assert.equal((await actions.readBlogAction({operation:'post',postId:'post',actor:'forged'})).code,'VALIDATION');
...
test('image upload actions preserve safe upload diagnostics and do not report draft-save failures', async () => {
  ...
  for (const [error, code, message] of [
    [new BlogImageUploadError('UPLOAD_NOT_CONFIGURED', 'Configure Cloudinary for image uploads.'), 'UPLOAD_NOT_CONFIGURED', 'Configure Cloudinary for image uploads.'],
    [new BlogImageUploadError('UPLOAD_REJECTED', 'Choose another image.'), 'UPLOAD_REJECTED', 'Choose another image.'],
    [new BlogImageUploadError('UPLOAD_FINALIZATION_FAILED', 'Image upload could not be recorded. Retry.'), 'UPLOAD_FINALIZATION_FAILED', 'Image upload could not be recorded. Retry.'],
