...
const image = {
  ...
  width: 800, height: 600, alt: 'A diagram with "labels"', caption: 'First line\nSecond line', displayWidth: 45, alignment: 'right',
};
...
function figure(metadata, src = blogEditorImageSource(image, 'blog-cloud'), extra = {}) {
  return {
    getAttribute(name) { return name === 'data-blog-image' ? metadata : (extra[name] ?? null); },
    querySelector() { return src === null ? null : { getAttribute(name) { return name === 'src' ? src : null; } }; },
  };
}
...
test('native image drops move the image in either direction, preserve metadata, and undo as one edit', () => {
  ...
  for (const target of [0, original.content.size]) {
    ...
    assert.deepEqual(state.doc.toJSON(), schema.nodes.doc.create(null,
      target === 0 ? [imageNode, before, after] : [before, after, imageNode],
    ).toJSON());
...
test('validated clipboard metadata cannot be overridden by arbitrary HTML attributes', () => {
  const parsed = parse(figure(JSON.stringify(image), undefined, { publicId: 'external', version: '-1', width: '-1', alt: 'changed', displayWidth: '999', alignment: 'absolute' }));
  assert.deepEqual(parsed, image);
  const { displayWidth, alignment, ...legacyImage } = image;
  assert.deepEqual(parse(figure(JSON.stringify(legacyImage))), { ...legacyImage, displayWidth: 100, alignment: 'center' });
});
...
test('clipboard parsing rejects untrusted URLs, malformed metadata, and values outside the image contract', () => {
  for (const src of [null, 'https://external.invalid/picture.png', blogEditorImageSource(image, 'another-cloud'), 'javascript:alert(1)', 'data:image/png;base64,AAAA']) {
    assert.equal(parse(figure(JSON.stringify(image), src)), false);
  }
  for (const metadata of ['', '{', 'null', '[]', ' '.repeat(6145), JSON.stringify({ ...image, padding: '汉'.repeat(2100) }), '{"__proto__":{},' + JSON.stringify(image).slice(1)]) {
    assert.equal(parse(figure(metadata)), false);
  }
  for (const attrs of [
    { publicId: '../outside' }, { version: 0 }, { version: 1.5 }, { format: 'svg' }, { width: 30001 }, { height: -1 },
    { displayWidth: 9 }, { displayWidth: 101 }, { displayWidth: 33.5 }, { alignment: 'absolute' },
    { alt: 'a'.repeat(501) }, { caption: 'c'.repeat(1001) }, { alt: '\u0000' }, { caption: '\ud800' }, { src: 'https://external.invalid/image.png' },
...
test('copied image JSON survives the plain Server Action payload and backend validation', async () => {
  ...
  let saved;
  const persistence = createDraftPersistence({ document, version: 1, onState() {}, request: async input => {
