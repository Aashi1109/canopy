...
test("authorization changes advance only affected user cache keys and roll back atomically", {
  ...
  const migration = await readFile(new URL("../packages/database/drizzle/0007_user_role_cache.sql", import.meta.url), "utf8");
  ...
  await assert.rejects(sql.begin(async (transaction) => {
    ...
    const timestamps = async () => Object.fromEntries((await transaction`
    ...
    await changesOnly(["a", "b"], () => transaction`INSERT INTO user_roles VALUES ('a', 'viewer'), ('b', 'viewer')`);
    ...
    await changesOnly(["b"], () => transaction`UPDATE roles SET access = '{"admin":{"enter":true}}' WHERE id = 'viewer'`);
    ...
    await assert.rejects(transaction.savepoint(async (savepoint) => {
...
test("concurrent role edits and assignments cannot leave a stale user cache key", {
  skip: enabled ? false : "set SMARTTOOLS_INTEGRATION=1 with a disposable DATABASE_URL",
}, async (context) => {
  const schema = `user_role_cache_test_${randomUUID().replaceAll("-", "")}`;
  ...
  await admin.begin(async (sql) => {
    ...
    `);
    await sql.unsafe(await readFile(new URL("../packages/database/drizzle/0007_user_role_cache.sql", import.meta.url), "utf8"));
  });
  const roleWriter = postgres(process.env.DATABASE_URL, { max: 1, connection: { search_path: schema }, onnotice() {} });
  const assignmentWriter = postgres(process.env.DATABASE_URL, { max: 1, connection: { search_path: schema }, onnotice() {} });
  ...
  const [{ updated_at: before }] = await assignmentWriter`SELECT updated_at FROM auth_users WHERE id = 'assignment-first'`;
  const roleUpdate = roleWriter`UPDATE roles SET access = '{}' WHERE id = 'editor'`.execute();
  await waitsFor(rolePid, assignmentPid);
  await assignmentWriter`COMMIT`;
  await roleUpdate;
  await roleWriter`COMMIT`;
  const [{ updated_at: after }] = await assignmentWriter`SELECT updated_at FROM auth_users WHERE id = 'assignment-first'`;
  assert.ok(after.toISOString() > before.toISOString(), "role update invalidates the just-committed assignment's cache key");
});
