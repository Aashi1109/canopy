...
import { parseCropPoints, selectionBounds, moveCropPoint, translateCrop, fullImagePoints, resizeCropPoints } from '../tools/crop-image/geometry.ts';
...
test('pre-run validation accepts freeform geometry and preserves rectangle validation', () => {
  const settings = { cropMode: 'freeform', cropPoints: JSON.stringify(fullImagePoints({ width: 100, height: 80 })), cropWidth: 0, cropHeight: 0 };
...
const points = [{ x: 10, y: 10 }, { x: 90, y: 5 }, { x: 80, y: 70 }, { x: 15, y: 60 }];
...
test('rejects malformed, crossing, degenerate, fractional and out-of-bounds points', () => {
  for (const raw of ['', 'not json', '{}', '[]', JSON.stringify([...points, points[0]]),
    JSON.stringify([{x:0,y:0},{x:100,y:80},{x:100,y:0},{x:0,y:80}]),
