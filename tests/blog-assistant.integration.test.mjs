...
test(
  ...
  async (t) => {
    ...
    await t.test("old Blog request fields and stored payloads are rejected without side effects", async () => {
    ...
    await t.test("five combined inputs replay and retry without recreating reference attachments", async () => {
    ...
    await t.test("title failure retains reported usage and article completion supplies a fallback title", async () => {
      ...
      try {
        ...
        assert.equal(
          (await pool.query("SELECT draft_document FROM blog_posts WHERE id=$1", [run.resourceId])).rows[0]
    ...
    await t.test(
      ...
      async () => {
        ...
        assert.equal(
          (await pool.query("SELECT count(*)::int n FROM assistant_attachments WHERE run_id=$1", [invalidId])).rows[0]
            .n,
          0,
        );
        assert.ok(
        ...
        await assert.rejects(
          assistant.removeAttachment("a", post.id, thread.id, writer.response.artifact.attachmentId),
          {
            code: "CONFLICT",
          },
        );
        ...
        assert.equal(
          (
            await pool.query(
              "SELECT count(*)::int n FROM assistant_attachments WHERE thread_id=$1 AND type='artifact'",
              [thread.id],
            )
          ).rows[0].n,
          0,
        ...
        assert.equal(
          (await pool.query("SELECT count(*)::int n FROM assistant_attachments WHERE run_id=$1", [afterClear.id]))
            .rows[0].n,
          0,
        );
        assert.equal(
