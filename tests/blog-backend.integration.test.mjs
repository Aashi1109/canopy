...
test('blog backend preserves drafts, snapshots, permissions and atomic publication', {
  ...
  for (const file of ['0001_auth_control_plane.sql', '0006_blogs.sql']) {
    await sql.unsafe(await readFile(new URL(`../packages/database/drizzle/${file}`, import.meta.url), 'utf8'));
  }
  ...
  const content = (title) => ({ ...createBlogDocument(title), excerpt: 'Useful guide', category: { id: category.id, label: category.name }, tags: [{ id: tag.id, label: tag.name }], body: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: `Body for ${title}` }] }] } });
  let post;
  await t.test('permission denial, forged fields, and concurrent slug collisions', async () => {
    ...
    const created = await Promise.all([m.createBlogPost('admin-a', { title: 'Same Title' }), m.createBlogPost('admin-a', { title: 'Same Title' })]);
  ...
  await t.test('taxonomy attribution is server assigned and no-op rename is stable', async () => {
    const renamed = await m.saveBlogTerm('admin-b', { kind: 'category', id: category.id, name: 'Tutorials' });
    assert.equal(renamed.createdBy, 'admin-a');
    assert.equal(renamed.updatedBy, 'admin-b');
    const same = await m.saveBlogTerm('admin-a', { kind: 'category', id: category.id, name: 'Tutorials' });
  ...
  await t.test('stale save cannot overwrite draft; slug is immutable and snapshot saves deduplicate', async () => {
    const initial = post;
    post = await m.saveBlogDraft('admin-a', { postId: post.id, version: post.version, document: content('First revision'), mode: 'manual' });
    assert.equal(post.slug, initial.slug);
    await assert.rejects(m.saveBlogDraft('admin-a', { postId: post.id, version: initial.version, document: content('Lost update') }), { code: 'CONFLICT' });
    await assert.rejects(m.saveBlogDraft('admin-a', { postId: post.id, version: post.version, document: { ...post.draftDocument, slug: 'changed' } }));
    const noOp = await m.saveBlogDraft('admin-a', { postId: post.id, version: post.version, document: post.draftDocument, mode: 'manual' });
    assert.equal(noOp.version, post.version);
    assert.equal(noOp.revisionSequence, post.revisionSequence);
  });
  let firstRevision;
  await t.test('publication and scheduled updates use immutable snapshots while newer drafts stay private', async () => {
    post = await m.publishBlogPost('admin-a', { postId: post.id, version: post.version });
    firstRevision = post.publishedRevisionId;
    post = await m.saveBlogDraft('admin-a', { postId: post.id, version: post.version, document: content('Scheduled version') });
    post = await m.scheduleBlogPost('admin-a', { postId: post.id, version: post.version, scheduledAt: new Date(Date.now() + 60000).toISOString() });
    const [frozen] = await sql`SELECT * FROM blog_post_schedules WHERE post_id=${post.id}`;
    post = await m.saveBlogDraft('admin-a', { postId: post.id, version: post.version, document: content('Private newer draft') });
    ...
    assert.equal(results.reduce((n,r) => n + r.published, 0), 1);
    ...
    post = { ...post, version: stored.version, publishedRevisionId: stored.published_revision_id };
  });
  await t.test('restoration backs up draft and preserves live identity; trash returns only to draft', async () => {
    ...
    post = await m.restoreBlogRevision('admin-a', { postId: post.id, version: post.version, revisionId: firstRevision });
    assert.equal(post.draftDocument.title, 'First revision');
    assert.equal(post.publishedRevisionId, live);
    const backups = await sql`SELECT document FROM blog_revisions WHERE post_id=${post.id} AND reason='restore_backup'`;
    ...
    post = await m.restoreTrashedBlogPost('admin-a', { postId: post.id, version: post.version });
    assert.equal(post.publishedRevisionId, null);
  ...
  await t.test('revoked publisher leaves recoverable diagnostics without changing draft/version', async () => {
    post = await m.scheduleBlogPost('admin-b', { postId: post.id, version: post.version, scheduledAt: new Date(Date.now()+60000).toISOString() });
