        ...
        >
          ...
          >
            <a
              ...
              href={blogListingHref(filters, { category: undefined, cursor: undefined })}
            >
              All stories
            </a>
          </Button>
      ...
      <BlogStories
            ...
            >
              ...
              >
                {filters.search ? (
                  ...
                  <>
                    <span className="lg:hidden">The latest</span>
                    <span className="hidden lg:inline">Latest articles</span>
                  </>
                )}
              </StoriesHeading>
              {!filters.search && (
                <Caption className="font-sans text-sm text-muted-foreground lg:text-[13px]">
                  <span className="lg:hidden">Good answers. No extra noise.</span>
        ...
        emptyState={
          <EmptyState
            className="my-6"
            icon={<BookOpen />}
            title={filters.search || filters.cursor ? "No stories found" : "Stories are on the way"}
            description={
              filters.search
                ? `No matches for “${filters.search}”. Try another keyword or browse all stories.`
                : filtered
                  ? "There are no published stories in this category yet. Explore the rest of the blog."
                  : filters.cursor
                    ? "You’ve reached the end. Return to the newest stories."
                    : "We’re preparing practical guides for your everyday work. Check back soon."
            }
            action={
              <Button asChild>
                <a
                  href={
                    filters.search
                      ? blogListingHref(filters, { search: undefined, cursor: undefined })
            ...
            <aside className="lg:col-start-2 lg:row-span-2 lg:row-start-1">
              <Card className="gap-1 rounded-lg border-0 bg-transparent p-0 shadow-none lg:bg-muted lg:p-6">
                <H2 className="font-sans text-2xl font-normal leading-normal lg:text-xl lg:font-semibold">
                  <span className="lg:hidden">Start with what you need to do</span>
                ...
                {categories.items.map((category) => (
                  ...
                  >
                    ...
                    <TextLink
                      ...
                      href={blogListingHref({}, { category: category.slug })}
                    >
                      {category.name} →
                    </TextLink>
                  </div>
