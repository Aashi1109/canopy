...
import {
  BlogError, createBlogPost, duplicateBlogPost, saveBlogDraft, publishBlogPost,
  scheduleBlogPost, retryBlogSchedule, cancelBlogSchedule, unpublishBlogPost, trashBlogPost,
  restoreTrashedBlogPost, restoreBlogRevision, saveBlogTerm,
...
import { getBlogPost, getBlogRevision, listBlogPosts, listBlogRevisions, listBlogTaxonomy } from '../../../../lib/blog/queries';
...
function failure(error: unknown) {
  ...
  if (error instanceof BlogValidationError || error instanceof ZodError) {
    return { ok: false as const, code: 'VALIDATION' as const, message: error instanceof ZodError ? 'Check the supplied fields and try again.' : error.message };
  }
  if (error instanceof AuthorizationError) return { ok: false as const, code: 'FORBIDDEN' as const, message: 'You do not have permission to perform this action.' };
  return { ok: false as const, code: 'TEMPORARY_FAILURE' as const, message: 'The change could not be saved. Try again.' };
}
...
export async function uploadBlogImageAction(formData: FormData) {
  ...
  try {
    ...
    return result.code === 'TEMPORARY_FAILURE'
      ? { ok: false as const, code: 'UPLOAD_TEMPORARY_FAILURE' as const, message: 'The image upload failed unexpectedly. Try uploading the image again.' }
      : result;
  }
}
...
const readInput = z.discriminatedUnion('operation', [
  ...
  z.object({ operation: z.literal('preview'), postId: z.string(), revisionId: z.string().optional() }).strict(),
  z.object({ operation: z.literal('taxonomy'), kind: z.enum(['category', 'tag']), filters: z.unknown().optional() }).strict(),
]);
...
export async function readBlogAction(input: unknown) {
  ...
  try {
    ...
    switch (value.operation) {
      case 'list': return { ok: true as const, data: await listBlogPosts(actor, value.filters) };
      case 'history': return { ok: true as const, data: await listBlogRevisions(actor, value.postId, value.cursor) };
      case 'taxonomy': return { ok: true as const, data: await listBlogTaxonomy(actor, value.kind, value.filters) };
      case 'post': {
      ...
      case 'preview': {
        ...
        return { ok: true as const, data: { document, ...renderBlogDocument(document, { cloudName: process.env.CLOUDINARY_CLOUD_NAME?.trim() }), robots: 'noindex, nofollow' } };
      }
    }
  } catch (error) {
    return failure(error);
