...
export interface TaxonomyOptions { items: { id: string; name: string }[]; nextCursor: string | null }

export function useBlogTaxonomyOptions(kind: "category" | "tag", initial: TaxonomyOptions, selected: { id: string; label: string }[] = []) {
  const [items, setItems] = useState(() => [...new Map([...selected.map(({ id, label }) => ({ id, name: label })), ...initial.items].map(item => [item.id, item])).values()]);
  ...
  async function loadMore() {
    ...
    } catch { setError("Couldn’t load more topics. Try again."); }
