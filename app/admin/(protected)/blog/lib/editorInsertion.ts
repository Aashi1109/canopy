...
export function createBlogTable(editor: Editor, columns: number, rows: number): JSONContent {
  if (![columns, rows].every(value => Number.isInteger(value) && value >= 1 && value <= 20)) throw new RangeError("Table columns and rows must each be between 1 and 20.");
...
export function captureBlogInsertion(editor: Editor, atEnd = false) {
  ...
  return {
    insert(content: JSONContent | JSONContent[]) {
      if (disposed || editor.isDestroyed || !editor.isEditable) { dispose(); return false; }
      const position = atEnd ? editor.state.doc.content.size : bookmark.resolve(editor.state.doc);
      dispose();
      return editor.commands.insertContentAt(typeof position === "number" ? position : { from: position.from, to: position.to }, content, { errorOnInvalidContent: true });
    },
    updateImage(attributes: { alt: string; caption: string }) {
      if (disposed || editor.isDestroyed || !editor.isEditable || originalImage?.type.name !== "image") { dispose(); return false; }
      ...
      return editor.commands.command(({ tr }) => { tr.setNodeMarkup(selection.from, undefined, { ...image.attrs, ...attributes }); return true; });
    },
    dispose,
  };
}
