...
export function createDraftPersistence<T>(options: {
  ...
  function clearTimers() { clearTimeout(idleTimer); clearTimeout(deadlineTimer); idleTimer = undefined; deadlineTimer = undefined; }
  function emit(state: DraftSaveState, message: string) { if (active) options.onState(state, message); }
  function remember() {
    if (!storage || !active) return;
    try { backupValue = JSON.stringify({ version: restoredVersion ?? version, document }); storage.setItem(storageKey, backupValue); } catch { options.onBackupFailure?.(); }
  }
  function forget() {
    try { if (backupValue && storage?.getItem(storageKey) === backupValue) storage.removeItem(storageKey); } catch { /* Private browser modes may disable storage. */ }
  }
  function schedule() {
    if (!active || conflict) return;
    clearTimeout(idleTimer);
    idleTimer = setTimeout(() => { void save("autosave"); }, 3_000);
    deadlineTimer ??= setTimeout(() => { void save("autosave"); }, 30_000);
  }
  async function save(mode: "manual" | "autosave" = "manual"): Promise<boolean> {
    ...
    pending = (async () => {
      try {
        ...
        const result = await Promise.resolve().then(() => options.request({ document: JSON.parse(JSON.stringify(snapshot)) as T, version, mode }));
        if (!result.ok) {
        ...
        return false;
      } finally { pending = null; pendingMode = null; }
    })();
  ...
  return {
    ...
    get dirty() { return generation !== savedGeneration; },
    attachStorage(nextStorage: Pick<Storage, "getItem" | "setItem" | "removeItem">, key: string): { version: number; document: unknown } | null {
      storage = nextStorage; storageKey = key;
      try {
        ...
        if (!value || typeof value !== "object" || !("version" in value) || !("document" in value) || typeof value.version !== "number" || !Number.isSafeInteger(value.version) || value.version < 1) return null;
        return { version: value.version, document: value.document };
      } catch (error) { if (!(error instanceof SyntaxError)) options.onBackupFailure?.(); return null; }
    },
    forgetBackup: forget,
    restore(value: T, baseVersion: number) {
      document = value; generation += 1; restoredVersion = baseVersion; remember();
      if (baseVersion !== version) { conflict = true; emit("conflict", "The saved post changed after this local draft. Download a backup before reloading."); return; }
      emit("dirty", "Recovered local draft · Unsaved changes"); schedule();
    },
    change(value: T) {
      document = value; generation += 1;
      remember();
      if (conflict) return;
      emit("dirty", "Draft · Unsaved changes");
      schedule();
    },
    markConflict(message: string) { conflict = true; clearTimers(); emit("conflict", message); },
    start() { active = true; if (generation !== savedGeneration) schedule(); },
    stop() { active = false; clearTimers(); },
  };
}
