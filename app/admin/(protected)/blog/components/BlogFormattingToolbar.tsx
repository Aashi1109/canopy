...
const HIGHLIGHT_COLORS = [["Green", "#dcfce7"], ["Blue", "#dbeafe"], ["Pink", "#fce7f3"], ["Purple", "#ede9fe"], ["Yellow", "#fef08a"]] as const;
...
function EditorMenuItem(props: ComponentProps<typeof Button>) {
  return <Button size="xs" variant="ghost" className="justify-start" {...props} />;
}

function BlogTablePicker({ onInsert, onBack, disabled, error }: { onInsert: (columns: number, rows: number) => void; onBack: () => void; disabled: boolean; error: string }) {
  const id = useId();
  ...
  function choose(column: number, row: number) { setSize({ columns: String(column), rows: String(row) }); }
  useEffect(() => { cells.current[18]?.focus(); }, []);
  return <div className="space-y-2 [@media(pointer:coarse)]:[&_input]:min-h-11 [@media(pointer:coarse)]:[&_button:not([role=gridcell])]:min-h-11">
    <div className="flex items-baseline justify-between gap-2"><p className="text-[13px] font-semibold">Insert table</p><span className="text-xs text-muted-foreground" aria-live="polite">{valid ? `${columns} × ${rows}` : "Choose a size"}</span></div>
    <div role="grid" aria-label="Table size, columns by rows" aria-rowcount={8} aria-colcount={8} className="space-y-0.5">
      {Array.from({ length: 8 }, (_, rowIndex) => <div role="row" key={rowIndex} className="grid grid-cols-8 gap-0.5">
        {Array.from({ length: 8 }, (_, columnIndex) => {
          const column = columnIndex + 1;
          const row = rowIndex + 1;
          const active = column === activeColumn && row === activeRow;
          return <Button key={column} ref={element => { cells.current[rowIndex * 8 + columnIndex] = element; }} role="gridcell" size="icon-sm" variant="outline" tabIndex={active ? 0 : -1} aria-colindex={column} aria-rowindex={row} aria-selected={active} aria-label={`${column} ${column === 1 ? "column" : "columns"}, ${row} ${row === 1 ? "row" : "rows"}`} disabled={disabled} data-highlighted={column <= columns && row <= rows} className="aspect-square h-auto w-full min-w-0 rounded-sm p-0 data-[highlighted=true]:border-primary data-[highlighted=true]:bg-primary/15" onPointerEnter={() => { if (!disabled) choose(column, row); }} onFocus={() => choose(column, row)} onClick={() => onInsert(column, row)} onKeyDown={event => {
            let nextColumn = column;
            let nextRow = row;
            switch (event.key) {
              case "ArrowLeft": nextColumn = Math.max(1, column - 1); break;
              case "ArrowRight": nextColumn = Math.min(8, column + 1); break;
              case "ArrowUp": nextRow = Math.max(1, row - 1); break;
              case "ArrowDown": nextRow = Math.min(8, row + 1); break;
              case "Home": nextColumn = 1; if (event.ctrlKey || event.metaKey) nextRow = 1; break;
              case "End": nextColumn = 8; if (event.ctrlKey || event.metaKey) nextRow = 8; break;
              default: return;
            }
    ...
    <div className="grid grid-cols-2 gap-2">
      <div className="space-y-1"><Label htmlFor={`${id}-columns`} className="text-xs">Columns</Label><Input id={`${id}-columns`} type="number" size="xs" min={1} max={20} step={1} value={size.columns} disabled={disabled} aria-invalid={!Number.isInteger(columns) || columns < 1 || columns > 20} onChange={event => setSize(previous => ({ ...previous, columns: event.target.value }))} /></div>
      <div className="space-y-1"><Label htmlFor={`${id}-rows`} className="text-xs">Rows</Label><Input id={`${id}-rows`} type="number" size="xs" min={1} max={20} step={1} value={size.rows} disabled={disabled} aria-invalid={!Number.isInteger(rows) || rows < 1 || rows > 20} onChange={event => setSize(previous => ({ ...previous, rows: event.target.value }))} /></div>
    </div>
    <p className="text-[11px] leading-4 text-muted-foreground">1–20 each · First row is a header.</p>
    {(error || !valid) && <p role="alert" className="text-[13px] text-destructive">{error || "Enter whole numbers from 1 to 20 for columns and rows."}</p>}
    <div className="flex justify-between gap-2"><Button size="xs" variant="ghost" onClick={onBack}>Back</Button><Button size="xs" disabled={disabled || !valid} onClick={() => onInsert(columns, rows)}>{valid ? `Insert ${columns} × ${rows}` : "Insert table"}</Button></div>
  </div>;
}
...
export function BlogBlockMenu({
  ...
  function clearInsertion() { pendingInsertion.current?.dispose(); pendingInsertion.current = null; }
  function backToBlocks() {
    clearInsertion(); setImageMode(false); setTableMode(false); setError("");
  ...
  function insert(type: string) {
    ...
    try {
      ...
      if (!inserted) { setError("Couldn’t insert the block. Select a place in the article and try again."); return; }
      keepEditorFocus.current = true;
      editor.commands.focus();
      setOpen(false);
    } catch { setError("Couldn’t insert the block. Select a place in the article and try again."); }
  }
  function insertTable(columns: number, rows: number) {
    ...
    try {
      ...
      if (!insertion.insert(table)) { setError("Couldn’t insert the table. Select a place in the article and try again."); return; }
      keepEditorFocus.current = true;
      editor.commands.focus();
      setOpen(false);
    } catch { setError("Couldn’t insert the table. Choose 1–20 columns and rows and try again."); }
  }
  return (
    <Popover.Root
      open={open}
    ...
    >
      ...
      <Popover.Portal>
        <Popover.Content
          ...
          sideOffset={8}
          collisionPadding={16}
          className={tableMode ? `${menuClass} w-60 max-w-[calc(100vw-32px)] p-2.5` : imageMode ? `${menuClass} w-80 max-w-[calc(100vw-32px)] space-y-3 p-3` : menuClass}
        ...
        >
          {tableMode ? <BlogTablePicker disabled={disabled || !editor?.isEditable} error={error} onInsert={insertTable} onBack={backToBlocks} /> : imageMode ? (
            <>
              ...
              {!editingImage && (
                <>
                  <Label className="text-[13px]" htmlFor={`blog-image-${atEnd}`}>Article image</Label>
                  <Input size="sm"
                    id={`blog-image-${atEnd}`}
                    type="file"
              ...
              <Label className="text-[13px]" htmlFor={`blog-alt-${atEnd}`}>Image description (required)</Label>
              <Input size="sm"
                id={`blog-alt-${atEnd}`}
                maxLength={500}
                value={alt}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    if (!editor || editor.isDestroyed || !editor.isEditable || disabled) return;
                    if (editingImage) {
                      const updated = pendingInsertion.current?.updateImage({ alt: alt.trim(), caption: caption.trim() });
                      clearInsertion();
                      if (!updated) { setError("The original image changed. Go back, select it, and reopen its description."); return; }
                      keepEditorFocus.current = true;
                      editor.commands.focus();
                      setOpen(false);
                      return;
                    ...
                    try {
                      ...
                      if (!inserted) { setError("Couldn’t insert the image. Select a place in the article and try again."); return; }
                      keepEditorFocus.current = true;
                      editor.commands.focus();
                      setOpen(false);
                    } catch {
            ...
            <div className="flex flex-col">
              {error && <p role="alert" className="max-w-64 px-2 text-[13px] text-destructive">{error}</p>}
              {([
              ...
              <EditorMenuItem
                onClick={() => {
                  ...
                  setCaption(
                    selected
                      ? String(editor?.getAttributes("image").caption ?? "")
                      : "",
                  );
                  setImageMode(true);
                }}
...
export function BlogFormattingToolbar({
  ...
  const state = useEditorState({
    ...
    selector: ({ editor: current }) => ({
      ...
      highlightColor: current?.isActive("highlight") ? String(current.getAttributes("highlight").color ?? "#fef08a") : null,
    ...
    <div
      ...
      onKeyDown={(event) => {
        if (!["ArrowLeft", "ArrowRight", "Home", "End"].includes(event.key))
          return;
        const buttons = Array.from(
          toolbar.current?.querySelectorAll<HTMLButtonElement>(
            "button:not(:disabled)",
          ) ?? [],
    ...
    >
      ...
      <div className={styles.formatGroup}>
        {menu("Text style", <Heading />, [
          ...
          ...([[2, Heading2], [3, Heading3], [4, Heading4], [5, Heading5], [6, Heading6]] as const).map(
            ([level, Icon]): [string, () => void, ReactNode] => [
              `Heading ${level}`,
      ...
      <div className={styles.formatGroup}>
        ...
        <div data-extra="true" className={styles.formatGroup}>
          ...
          <Popover.Root open={highlightOpen && !unavailable} onOpenChange={setHighlightOpen}>
            <Tooltip><TooltipTrigger asChild><Popover.Trigger asChild><Button
              ...
              variant="ghost"
              onMouseDown={event => event.preventDefault()}
            ><Highlighter /></Button></Popover.Trigger></TooltipTrigger><TooltipContent>Highlight color</TooltipContent></Tooltip>
            ...
            >
              <BlogColorPalette colors={HIGHLIGHT_COLORS} label="Highlight" separateClear value={state?.highlightColor ?? null} disabled={unavailable} onChange={color => {
                if (!editor || editor.isDestroyed || !editor.isEditable || unavailable) return;
                const chain = editor.chain().focus();
                const applied = color ? chain.setMark("highlight", { color }).run() : chain.unsetMark("highlight").run();
                if (applied) {
        ...
        >
          ...
          <Popover.Portal>
            ...
            >
              <Label className="text-[13px]" htmlFor="blog-inline-link">Link URL</Label>
              <Input size="sm"
                id="blog-inline-link"
                type="url"
                value={url}
