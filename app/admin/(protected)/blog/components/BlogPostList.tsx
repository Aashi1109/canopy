  ...
  return (
    <TooltipProvider>
      <section aria-busy={busy} className={`${styles.posts} flex min-w-0 flex-col gap-6`}>
          ...
          onValueChange={(value) => {
            if (
              ...
              onFiltersChange({
                ...filters,
                status: value,
                category: trash ? "all" : filters.category,
              });
            }
          }}
        >
        ...
        <form
          className="grid items-end gap-4 md:grid-cols-[minmax(0,1fr)_240px_auto]"
          onSubmit={(event) => {
                ...
                <TableRow key={post.id}>
                  <TableCell className="max-w-64 py-4 sm:max-w-none">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground md:hidden">
                      {STATUS[post.status].label} · {post.updatedLabel}
                    </p>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className={`flex justify-end gap-1 ${styles.rowActions}`}>
                      <Button asChild variant="ghost" className="md:hidden">
                        <Link href={post.editHref}>Edit post</Link>
                      </Button>
                      <Tooltip>
                      ...
                      {canArchive && (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              disabled={busy}
                              aria-label={`${trash ? "Restore" : "Move to trash:"} ${post.title}`}
                              className={trash ? undefined : "text-destructive"}
                              size="icon-sm"
                              variant="ghost"
                              onClick={() => (trash ? onRestore(post) : onTrash(post))}
                            >
                        ...
                        <Popover.Root>
                          ...
                          <Popover.Portal>
                            <Popover.Content
                              align="end"
                              sideOffset={6}
                              className="z-50 grid w-60 gap-1 rounded-lg border border-border bg-popover p-2 shadow-md"
                            >
                              ...
                              <Button asChild variant="ghost" className="justify-start">
                                <Link href={post.historyHref}>Revision history</Link>
                              </Button>
                              {canArchive && (
                                <Popover.Close asChild>
                                  <Button
          ...
          <nav aria-label="Post pages" className="flex gap-2">
            {pagination.previousHref ? (
              <Button asChild size="sm" variant="outline">
                <Link href={pagination.previousHref}>{pagination.previousLabel ?? "Previous"}</Link>
              </Button>
            ) : (
