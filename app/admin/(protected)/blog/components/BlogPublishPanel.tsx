...
export function BlogPublishPanel({
  ...
  return (
    <form
      className={`${styles.publishPanel} space-y-5`}
      aria-busy={pending}
      onSubmit={(event) => {
    ...
    >
      ...
      <div className="grid gap-2">
        ...
        <Select
          ...
          disabled={pending}
          onValueChange={(value) => {
            if (value === "now" || value === "schedule") onModeChange(value);
          }}
        >
          <SelectTrigger size="sm" id="blog-publish-mode" className="w-full">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem className="text-[13px]" value="now">
              Publish now
            </SelectItem>
            <SelectItem className="text-[13px]" value="schedule">
              Schedule
            </SelectItem>
          </SelectContent>
        </Select>
      ...
      {mode === "schedule" && (
        <>
          <div className="grid grid-cols-[minmax(0,1fr)_136px] gap-4">
            <div className="grid gap-2">
              <Label className="text-[13px]" htmlFor="blog-publish-date">
                Publication date
              </Label>
              <Input
                size="sm"
                ...
                disabled={pending}
                value={schedule.date}
                onChange={(event) => onScheduleChange({ ...schedule, date: event.target.value })}
              />
            </div>
            <div className="grid gap-2">
              <Label className="text-[13px]" htmlFor="blog-publish-time">
                Time (24-hour)
              </Label>
              <Select
                required
                value={schedule.time}
                disabled={pending}
                onValueChange={(time) => onScheduleChange({ ...schedule, time })}
              >
                <SelectTrigger size="sm" id="blog-publish-time" className="w-full">
                  <SelectValue placeholder="Select time" />
                </SelectTrigger>
                <SelectContent>
                  {TIMES.map((time) => (
                    <SelectItem className="text-[13px]" key={time} value={time}>
                      {time}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <div className="grid gap-2">
            <Label className="text-[13px]" htmlFor="blog-publish-timezone">
              Timezone
            </Label>
            <Select
              required
              ...
              disabled={pending}
              onValueChange={(timezone) => onScheduleChange({ ...schedule, timezone })}
            >
              <SelectTrigger size="sm" id="blog-publish-timezone" className="w-full">
                <SelectValue placeholder="Select timezone" />
              </SelectTrigger>
              <SelectContent>
                {timezones.map((timezone) => (
                  <SelectItem className="text-[13px]" key={timezone} value={timezone}>
                    {timezone}
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      <Accordion
        type="single"
        collapsible
        hidden={mode === "schedule" && ready}
        key={ready ? "ready" : "needs-attention"}
          ...
          <AccordionContent>
            <ul className="space-y-2">
              {checks.map((check) => (
                <li
                  key={check.id}
                  className={`flex items-start gap-2 ${check.valid ? "text-success" : "text-destructive"}`}
                >
                  {check.valid ? (
            ...
            {failedChecks[0] && (
              <Button
                ...
                variant="outline"
                disabled={pending}
                onClick={() => onFixCheck(failedChecks[0].id)}
              >
                Fix {failedChecks[0].label.toLowerCase()}
              </Button>
            )}
          </AccordionContent>
        </AccordionItem>
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-[13px] font-semibold">Search preview</h3>
          <Button size="xs" variant="ghost" onClick={onEditSeo} disabled={pending}>
            Edit SEO
          </Button>
        </div>
      ...
      <AlertBanner
        title={
          mode === "schedule"
            ? hasLiveVersion
              ? "Current article stays live"
              : "Not public yet"
            : "Public immediately"
        }
      >
        {mode === "schedule"
          ? `${hasLiveVersion ? "The current article stays public until this update publishes. " : ""}Only this saved revision will go live. Later edits won’t change the scheduled version.`
          : "This saved revision will be visible to everyone. You can unpublish it later."}
      </AlertBanner>
      {error && (
        <AlertBanner variant="error" title="Couldn’t publish">
      ...
      >
        ...
        <Button size="sm" type="submit" loading={pending} disabled={!canPublish || !ready}>
