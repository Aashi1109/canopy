...
import { ArrowLeft, Copy, ExternalLink, EyeOff, CalendarX, RotateCw, Pencil, Replace, Trash2 } from "lucide-react";
import { z } from "zod";
...
} from "../lib/useBlogTaxonomyOptions";
import styles from "./BlogEditor.module.css";

function bodyImages(node: JSONContent): JSONContent[] {
  return node.type === "image"
    ? [node]
    : (node.content ?? []).flatMap(bodyImages);
}
...
export function BlogEditor({
  ...
  const tagOptions = useBlogTaxonomyOptions(
    "tag",
    initialTags,
    post.draftDocument.tags,
  );
  ...
  const [persistence] = useState(() =>
    createDraftPersistence<EditableDocument>({
      ...
      request: async (input) => {
        ...
        if (result.ok && !("version" in result.data))
  ...
  useEffect(() => {
    ...
    return () => { uploadActive.current = false; toast.dismiss(uploadToastId); };
  }, [uploadToastId]);
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor: changed }) =>
      change({ ...current.current, body: changed.getJSON() }),
  });
  useEffect(() => {
  ...
  async function uploadImage(file: File, target: "cover" | "body"): Promise<BlogImage | null> {
    ...
    try {
      ...
      showUploadError(
        "Couldn’t upload the image. Try a smaller image or try again.",
      );
      return null;
    } finally {
  ...
  async function openPreview() {
    ...
    if (
      editable &&
      (persistence.dirty || saveState === "saving" || saveState === "error")
    ) {
  ...
  async function updatePublication() {
    ...
    try {
      if (editable && !(await persistence.save())) {
        setLifecycleError(
          "Save or recover your draft before changing publication.",
        );
        return;
      }
      ...
      if (!result.ok) {
        if (result.code === "CONFLICT")
          persistence.markConflict(result.message);
      ...
      setPublication({
        published:
          ...
          (lifecycle !== "unpublish" && publication.published),
        scheduled: false,
        scheduledAt: null,
        error: null,
      });
  ...
  async function publish() {
    ...
    try {
        ...
        mode === "schedule"
          ? await mutateBlogAction("schedule", {
              ...
              scheduledAt: new Date(
                `${schedule.date}T${schedule.time}:00Z`,
              ).toISOString(),
            })
      ...
      setPublishError(
        "The request failed. Check the date and time, then try again.",
      );
    } finally {
      inFlight.current = false;
      setPublishing(false);
  ...
  function fixPublishCheck(id: string) {
    ...
    setSettingsTab("post");
    setReturnSettingsOpen(
      ["excerpt", "author", "category"].includes(id),
    );
      ...
      <div className={styles.review} data-schedule={mode === "schedule"}>
        <header className={styles.reviewHeader}>
          <Button size="sm"
            variant="ghost"
            onClick={() =>
              mode === "schedule" ? setMode("now") : setReview(false)
            }
            disabled={publishing}
          >
        ...
        <div className={styles.reviewWorkspace}>
          <div className={styles.recap}>
            ...
            <p className="text-[13px] text-muted-foreground">
              {document.authorName} ·{" "}
              {document.category?.label ?? "No category"}
            </p>
            <p className={`${styles.recapLabel} text-xs text-muted-foreground`}>
              PUBLIC URL
            </p>
            <p className="break-all text-base text-primary">
              /blog/{post.slug}
            </p>
          </div>
          <BlogPublishPanel
  ...
  const coverPreview = document.coverImage && <img
    src={imageSource(document.coverImage, cloudName)}
    alt={document.coverImage.alt}
    width={document.coverImage.width}
    height={document.coverImage.height}
  ...
  return (
      ...
      publicationActions={
        <>
          ...
          {canPublish && publication.scheduled && (
            ...
            >
              <CalendarX aria-hidden="true" />Cancel schedule
            </Button>
          )}
          {canPublish && publication.error && (
            ...
            >
              <RotateCw aria-hidden="true" />Retry scheduled publication
            </Button>
          )}
        </>
      }
      history={<BlogHistoryPanel currentTitle={document.title} postId={post.id} version={persistence.version} publishedRevisionId={post.publishedRevisionId} />}
      toolbar={
          ...
          onChange={(value) =>
            change({
              ...
              tags: value.tagIds.flatMap((id) => {
                ...
                const existing = current.current.tags.find(
                  (tag) => tag.id === id,
                );
                return term
                  ? [{ id, label: term.name }]
    ...
    >
      ...
      {(saveState === "conflict" || saveState === "error") && (
        ...
        >
          Your local edits are preserved. Download a backup before leaving or
          reloading.
      ...
      {lifecycleError && !lifecycle && (
        <AlertBanner variant="error">{lifecycleError}</AlertBanner>
      )}
      {backupUnavailable && (
        <AlertBanner variant="warning">
          This browser couldn’t keep a local recovery copy. Wait for “All
          changes saved” before leaving the editor.
        </AlertBanner>
      )}
      ...
      <Textarea
        ...
        onChange={(event) =>
          change({ ...current.current, excerpt: event.target.value })
        }
      />
      <div className="my-[22px] space-y-3">
        {document.coverImage && <Popover.Root open={editable && !publishing && !recovery && coverOpen} onOpenChange={setCoverOpen}>
          <div>
          {editable ? <Popover.Trigger asChild><Button
            variant="ghost"
            className={`${styles.coverTrigger} block h-auto w-full rounded-lg p-0 disabled:opacity-100`}
            aria-label="Edit cover image"
            disabled={uploading || publishing || !!recovery}
          >{coverPreview}</Button></Popover.Trigger> : coverPreview}
          {editable && <div className="mt-2 flex flex-wrap items-center gap-2 [@media(pointer:coarse)]:[&_button]:min-h-11">
            <Button id="blog-edit-cover" variant="outline" size="xs" disabled={uploading || publishing || !!recovery} onClick={() => setCoverOpen(true)}><Pencil aria-hidden="true" />Edit cover</Button>
            {!document.coverImage.alt.trim() && <Button variant="ghost" size="xs" className="font-normal text-destructive" disabled={uploading || publishing || !!recovery} onClick={() => setCoverOpen(true)}>Alt text missing</Button>}
          </div>}
          </div>
          <Popover.Portal><Popover.Content
            ...
            side="bottom" align="center" sideOffset={4} collisionPadding={12} aria-label="Cover image settings"
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <Input id="blog-cover-alt" size="sm" maxLength={500} aria-required="true" aria-describedby="blog-cover-alt-help" value={document.coverImage.alt} disabled={uploading || publishing || !!recovery} placeholder="Describe what the image shows" onChange={event => {
                const cover = current.current.coverImage;
                if (cover) change({ ...current.current, coverImage: { ...cover, alt: event.target.value } });
              }} />
              <p id="blog-cover-alt-help" className="text-xs text-muted-foreground">Describe the image for screen readers. Required before publishing; saves with your draft.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" loading={uploadTarget === "cover"} disabled={uploading || publishing || !!recovery} onClick={() => coverInput.current?.click()}><Replace aria-hidden="true" />Replace cover</Button>
              <Button variant="danger-subtle" size="sm" disabled={uploading || publishing || !!recovery} onClick={() => {
                setCoverOpen(false);
                change({ ...current.current, coverImage: null });
              }}><Trash2 aria-hidden="true" />Remove cover</Button>
            </div>
            </div>
      ...
      {uploadTarget === "body" && <p role="status" className="mb-2 text-[13px] text-muted-foreground">Uploading article image…</p>}
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <Button size="sm"
              variant={
                lifecycle === "retrySchedule" ? "default" : "destructive"
              }
              loading={publishing}
              onClick={() => {
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Your latest edits will be lost. Stay here and save your draft
              before leaving.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <Button size="sm"
              ...
              onClick={() => {
                ...
                if (leaveHref === `/admin/blog/${post.id}`)
