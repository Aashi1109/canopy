...
import { Bold, Italic, Link2, List, ListOrdered, NotebookPen, ScanSearch, Sparkles, SquarePen, X } from "lucide-react";
import { Button, Popover, Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/index.tsx";
import { parseComposerContent } from "@/lib/blog/composerDocument";
import { getBlogAgent } from "@/lib/blog/agentCatalog";
import { BlogEditorLinkForm } from "./BlogLinkForm";
import styles from "./BlogEditor.module.css";

const AGENT_ICONS = { planner: NotebookPen, writer: SquarePen, auditor: ScanSearch, optimizer: Sparkles };

function AgentMention({ node, deleteNode, selected }: NodeViewProps) {
  const agent = getBlogAgent(node.attrs.agentId);
  const Icon = agent ? AGENT_ICONS[agent.id] : Sparkles;
  return (
...
const Agent = Node.create({
  ...
  renderHTML: ({ node }) => [
    "span",
    { "data-agent-id": node.attrs.agentId },
    getBlogAgent(node.attrs.agentId)?.name ?? "Agent",
  ],
  addNodeView: () => ReactNodeViewRenderer(AgentMention),
});

export type BlogAgentComposerHandle = {
  focus: () => void;
...
type Props = {
  ref?: Ref<BlogAgentComposerHandle>;
  value: string;
  content?: JSONContent;
  agentId?: string;
  agentOffset?: number;
...
export function BlogAgentComposer(props: Props) {
  ...
  useEffect(() => {
    ...
    editor.setOptions({
      editorProps: {
        ...
        attributes: {
          id: "blog-assistant-message",
          role: "combobox",
          "aria-label": "Message to assistant",
          "aria-multiline": "true",
          "aria-autocomplete": "list",
          "aria-expanded": String(props.expanded),
          ...(props.expanded ? { "aria-controls": "blog-agent-commands" } : {}),
          ...(props.activeDescendant ? { "aria-activedescendant": props.activeDescendant } : {}),
          "aria-describedby": [props.describedBy, limitError ? "composer-limit-error" : ""].filter(Boolean).join(" "),
  ...
  return (
    <>
      ...
      {limitError && (
        <p id="composer-limit-error" role="status" className="text-caption text-destructive">
          ...
          >
            ...
            <Popover.Portal>
              <Popover.Content
                ...
                sideOffset={8}
              >
                <BlogEditorLinkForm editor={editor} onClose={() => setLinkOpen(false)} />
              </Popover.Content>
            </Popover.Portal>
          </Popover.Root>
        </BubbleMenu>
