...
export function BlogColorPalette({ colors, label, value, onChange, disabled = false, separateClear = false }: {
  ...
  return <div className="flex flex-wrap items-center gap-2 [@media(pointer:coarse)]:[&_button]:min-h-11 [@media(pointer:coarse)]:[&_button]:min-w-11" role="group" aria-label={label}>
    {colors.map(([name, color]) => <Tooltip key={color}>
    ...
    <div className={separateClear ? "border-l border-border pl-2" : undefined}>
      ...
      ><CircleOff aria-hidden="true" className={separateClear ? "size-6 [@media(pointer:coarse)]:size-11" : undefined} /></Button></TooltipTrigger><TooltipContent>Clear {label.toLowerCase()}</TooltipContent></Tooltip>
    </div>
  </div>;
}
