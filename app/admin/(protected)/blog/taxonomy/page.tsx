...
export default async function BlogTaxonomyPage({
  ...
  searchParams: Promise<{ kind?: string; cursor?: string; returnTo?: string }>;
}) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <Button asChild variant="ghost">
        <Link href={editorHref ?? "/admin/blog"}>
          <ArrowLeft aria-hidden="true" />
          {editorHref ? "Back to editor" : "Posts"}
        </Link>
      </Button>
      <div>
      ...
      <nav aria-label="Taxonomy type" className="flex gap-2">
        ...
        <Button asChild variant={kind === "tag" ? "secondary" : "ghost"}>
          <Link href={taxonomyHref("tag")} aria-current={kind === "tag" ? "page" : undefined}>
            Tags
          </Link>
        </Button>
      </nav>
      <BlogTaxonomy key={kind} kind={kind} terms={terms.items} returnTo={editorHref} />
      <nav aria-label="Taxonomy pages" className="flex justify-end gap-3">
        {cursor && (
          <Button asChild size="sm" variant="outline">
            <Link href={taxonomyHref(kind)}>First page</Link>
          </Button>
        )}
        {terms.nextCursor && (
          <Button asChild size="sm" variant="outline">
            <Link href={taxonomyHref(kind, terms.nextCursor)}>Next</Link>
          </Button>
        )}
      </nav>
    </div>
