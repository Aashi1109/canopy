...
function getPlan(settings: WorkspaceProps["settings"], pageCount: number) {
  ...
  return {
    ...
    detail: <div className="max-h-32 overflow-y-auto text-muted-foreground">Pages in output order: {pages.join(", ")}</div>,
  };
}

export default function ExtractPdfWorkspace(props: WorkspaceProps) {
  return <PdfFileWorkspace {...props} definitionKey="extract-pdf-pages" optionsTitle="Extraction settings" getPlan={getPlan}
    ...
    renderPageOverlay={({ pageNumber }, pages) => {
      ...
      return <PdfPageSelectionOverlay
        pageNumber={pageNumber}
        selected={selected}
        disabled={props.disabled}
        onToggle={() => props.onSettingChange("pages", (selected ? selection.filter((page) => page !== pageNumber) : [...selection, pageNumber]).join(","))}
      />;
    }}
  />;
}
