...
function getPlan(settings: WorkspaceProps["settings"], pageCount: number) {
  ...
  const pageSize = settings.pageSize === "letter" || settings.pageSize === "legal" || settings.pageSize === "custom" ? settings.pageSize : "a4";
  const target = pdfSize(pageSize, Number(settings.width ?? 595), Number(settings.height ?? 842));
  if (!Number.isFinite(target.width) || !Number.isFinite(target.height)) throw new Error("Enter valid page dimensions.");
