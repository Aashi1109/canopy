...
import { fullImagePoints, moveCropPoint, parseCropPoints, selectionBounds, resizeCropPoints, type CropPoint } from "./geometry";
...
function CropPreview({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      {url ? (
        <div className="relative inline-block max-w-full shrink-0 overflow-hidden touch-none select-none">
          <img
            ...
            draggable={false}
            onLoad={(event) => {
              const next = { height: event.currentTarget.naturalHeight, width: event.currentTarget.naturalWidth };
              setSize(next);
              onImageLoad?.(next);
            }}
            src={url}
      ...
      <Muted className="shrink-0 border-t border-border px-4 py-2">Arrow keys move 1 px · Shift moves 10 px · Drag the corner to resize</Muted>
    </div>
  );
}
...
function CropEditor(props: WorkspaceProps) {
  ...
  try { points = parseCropPoints(settings.cropPoints, size.width ? size : undefined); } catch { /* Not decoded or invalid yet. */ }
  ...
  useEffect(() => {
    if (size.width && size.height && !settings.cropPoints) {
      try { writePoints(resizeCropPoints(fullImagePoints(size), pointCount, size)); }
      catch (error) { setPointIssue(error instanceof Error ? error.message : "Choose fewer points."); }
    }
  ...
  const outputFields = Object.fromEntries(Object.entries(fields).filter(([key]) =>
    key === "outputFormat" || (key === "quality" && settings.outputFormat !== "png" && !(settings.outputFormat === "original" && file?.type === "image/png"))));
  const resetCrop = () => {
    setPointIssue("");
    if (freeform) {
      try { writePoints(resizeCropPoints(fullImagePoints(size), pointCount, size)); }
    ...
    else { change("cropX", 0); change("cropY", 0); change("cropWidth", size.width); change("cropHeight", size.height); change(ASPECT, FREE); }
  };
  return (
    <FileProcessorWorkspace {...props} compactFileToolbar onSettingChange={onSettingChange}
      onInputChange={input => {
      ...
      renderOptions={() => <>
        <div className={freeform ? "grid grid-cols-2 items-start gap-4" : undefined}>
          <SettingsPanel disabled={props.disabled} onChange={onSettingChange} spec={{ fields: { cropMode: fields.cropMode } }} values={settings} />
          {freeform ? <div className="grid gap-1.5">
            <FieldLabel htmlFor={`${fieldId}-count`}>Freeform point count</FieldLabel>
            <Input id={`${fieldId}-count`} type="number" min={3} max={12} step={1} disabled={props.disabled}
              value={countDraft ?? pointCount} aria-describedby={`${fieldId}-count-help`}
              onBlur={() => { if (countDraft !== null) { setCountDraft(null); setPointIssue(""); } }}
              onChange={event => {
                setCountDraft(event.currentTarget.value);
                const value = event.currentTarget.valueAsNumber;
                if (!Number.isInteger(value) || value < 3 || value > 12) { setPointIssue("Choose between 3 and 12 points."); return; }
                try {
                  if (size.width && size.height) writePoints(resizeCropPoints(points.length >= 3 ? points : fullImagePoints(size), value, size));
                  change("cropPointCount", value); setCountDraft(null); setSelected(0); setPointIssue("");
                } catch (error) { setPointIssue(error instanceof Error ? error.message : "Choose fewer points."); }
              }} />
        ...
        {freeform ? <>
          ...
          {(["x", "y"] as const).map(axis => <div className="grid gap-2" key={axis}>
            <FieldLabel htmlFor={`${fieldId}-${axis}`}>{axis.toUpperCase()} · selected point {selected + 1}</FieldLabel>
            <Input id={`${fieldId}-${axis}`} suffix="px" type="number" min={0} max={axis === "x" ? size.width : size.height} step={1}
              disabled={props.disabled || points.length < 3 || !size.width} value={points[selected]?.[axis] ?? ""}
              aria-invalid={Boolean(pointIssue)} aria-describedby={pointIssue ? `${fieldId}-issue` : undefined}
              onChange={event => {
                const value = event.currentTarget.valueAsNumber;
                if (!Number.isSafeInteger(value) || !points[selected]) { setPointIssue("Enter a whole-number pixel position."); return; }
                const next = moveCropPoint(points, selected, { ...points[selected], [axis]: value }, size);
                if (next === points) { setPointIssue("Points cannot overlap or cross the opposite edge."); return; }
                setPointIssue(""); writePoints(next);
              }} />
          </div>)}
          {(["width", "height"] as const).map(axis => <div className="grid gap-2" key={axis}>
            <FieldLabel htmlFor={`${fieldId}-${axis}`}>{axis === "width" ? "Width" : "Height"} · selection bounds</FieldLabel>
            <Input id={`${fieldId}-${axis}`} suffix="px" readOnly value={bounds[axis]} />
          </div>)}
          </div>
          {pointIssue ? <Muted id={`${fieldId}-issue`} role="alert">{pointIssue}</Muted> : null}
          <Muted>Select a point to edit its X and Y. Width and height show the selection’s bounding box.</Muted>
        </> : <SettingsPanel className="grid-cols-2" layout="grid" disabled={props.disabled} onChange={onSettingChange} values={settings}
          spec={{ fields: Object.fromEntries(Object.entries(fields).filter(([key]) => [ASPECT, ...Object.values(CROP_KEYS)].includes(key))) }} />}
        <SettingsPanel disabled={props.disabled} onChange={onSettingChange} spec={{ fields: outputFields }} values={settings} />
        {freeform ? <Muted>{settings.outputFormat === "jpeg" || (settings.outputFormat === "original" && file?.type === "image/jpeg")
          ? "JPEG fills the area outside your selection with white. Choose PNG or WebP to keep it transparent."
          : "Pixels outside the selection are transparent. This crops the shape; it does not straighten perspective."}</Muted> : null}
        <Button disabled={props.disabled || !file || !size.width} onClick={resetCrop} variant="outline">Reset crop</Button>
      </>}
      detail={({ disabled }) => freeform ? (imageError ? <div role="alert" className="p-4"><Muted>{imageError} Replace the image to try again.</Muted></div> : url ?
        <FreeformPreview url={url} points={points} size={size} selected={selected} disabled={disabled}
          onInvalidMove={() => setPointIssue("Points cannot overlap or cross the opposite edge.")}
          onSelect={index => { setSelected(index); setPointIssue(""); }} onChange={next => { setPointIssue(""); writePoints(next); }}
          onError={() => { setImageError("This image could not be decoded."); change("cropPoints", ""); setSize(NO_SIZE); }}
          onLoad={next => {
            if (!next.width || !next.height || next.width * next.height > 100_000_000) {
              setImageError("Choose an image under 100 megapixels."); change("cropPoints", ""); setSize(NO_SIZE); return;
            }
            setSize(next);
          }} /> : <Muted>Loading image…</Muted>) : (
        <CropPreview disabled={disabled} file={file} onSettingChange={props.onSettingChange} settings={settings} onImageLoad={setSize} />
      )}
    />
  );
}
