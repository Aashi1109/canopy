...
function intersects(a: CropPoint, b: CropPoint, c: CropPoint, d: CropPoint): boolean {
  ...
  const on = (p: CropPoint, q: CropPoint, r: CropPoint) =>
    cross(p, q, r) === 0 &&
      ...
      .map((p, i) => {
        ...
        return {
          score: adding
            ...
            : Math.abs(cross(next[(i + next.length - 1) % next.length], p, q)),
          points: adding
            ? [...next.slice(0, i + 1), middle, ...next.slice(i + 1)]
            : next.filter((_, index) => index !== i),
        };
      })
      .sort((a, b) => a.score - b.score);
    const valid = candidates.find((candidate) => {
      try {
        parseCropPoints(candidate.points, size);
        return true;
      } catch {
        return false;
      }
...
export function moveCropPoint(
  ...
  try {
    return parseCropPoints(next, size);
  } catch {
    return points;
  }
}
