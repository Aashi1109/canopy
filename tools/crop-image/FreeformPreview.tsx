...
import { Muted } from "@smarttools/ui";
...
export function FreeformPreview({ url, points, size, selected, disabled, onSelect, onChange, onLoad, onError, onInvalidMove }: Props) {
  const hintId = useId();
  const maskId = useId().replace(/:/g, "");
  const surface = useRef<HTMLDivElement>(null);
  const drag = useRef<{ id: number; index: number | null; start: CropPoint; points: readonly CropPoint[] } | null>(null);
  ...
  const move = (event: PointerEvent) => {
    ...
    const next = current.index === null
      ...
      : moveCropPoint(current.points, current.index, {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <Muted className="shrink-0 px-4 py-2" id={hintId}>Drag handles to shape the crop · Drag inside to move it</Muted>
      <div className="flex min-h-0 flex-1 items-start justify-center overflow-auto p-6">
        <div ref={surface} className="relative inline-block max-w-full shrink-0 touch-none select-none"
          onKeyDown={(event) => { if (event.key === "Escape" && drag.current) { event.preventDefault(); finish(true); } }}
          onPointerMove={move} onPointerUp={() => finish()} onPointerCancel={() => finish(true)}
          onLostPointerCapture={() => { if (drag.current) finish(true); }}>
          <img alt="Source image with freeform crop selection" className="block max-h-[520px] max-w-full object-contain outline outline-1 outline-black/10 dark:outline-white/10"
            draggable={false} src={url} onError={onError}
            onLoad={event => onLoad({ width: event.currentTarget.naturalWidth, height: event.currentTarget.naturalHeight })} />
          {ready ? <>
            <svg aria-label="Crop selection" className="absolute inset-0 h-full w-full overflow-visible" viewBox={`0 0 ${size.width} ${size.height}`}>
              <defs><mask id={maskId}><rect width={size.width} height={size.height} fill="white" /><polygon points={polygon} fill="black" /></mask></defs>
              <rect width={size.width} height={size.height} fill="black" opacity="0.4" mask={`url(#${maskId})`} pointerEvents="none" />
              <polygon aria-label="Move entire crop selection" aria-describedby={hintId} role="button" tabIndex={disabled ? -1 : 0}
                ...
                points={polygon} strokeWidth="2" vectorEffect="non-scaling-stroke"
                onPointerDown={event => start(event, null)}
                onKeyDown={event => {
                  if (disabled || !event.key.startsWith("Arrow")) return;
                  event.preventDefault();
                  const step = event.shiftKey ? 10 : 1;
                  onChange(translateCrop(points, event.key === "ArrowLeft" ? -step : event.key === "ArrowRight" ? step : 0,
                    event.key === "ArrowUp" ? -step : event.key === "ArrowDown" ? step : 0, size));
                }} />
            </svg>
            {points.map((point, index) => (
              <button key={index} type="button" aria-label={`Crop point ${index + 1}`} aria-pressed={selected === index}
                aria-describedby={hintId} disabled={disabled}
                ...
                onFocus={() => onSelect(index)} onPointerDown={event => start(event, index)}
                onKeyDown={event => {
                  if (disabled || !event.key.startsWith("Arrow")) return;
                  event.preventDefault();
                  const step = event.shiftKey ? 10 : 1;
                  const next = moveCropPoint(points, index, {
                ...
                <span className={`size-4 rounded-full border-2 border-white bg-primary ${selected === index ? "ring-4 ring-primary/25" : ""}`} />
              </button>
            ))}
          </> : null}
        </div>
