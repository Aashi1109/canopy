...
export function measurementId(env: { NODE_ENV?: string; VERCEL_ENV?: string; GA_MEASUREMENT_ID?: string; GA_ENABLE_IN_DEVELOPMENT?: string }) {
  const id = env.GA_MEASUREMENT_ID?.trim();
  const enabled = env.NODE_ENV === "production" || (env.NODE_ENV === "development" && env.GA_ENABLE_IN_DEVELOPMENT === "true");
...
export function publicPath(pathname: string): string | null {
  ...
  if (["/", "/privacy", "/contact", "/media", "/devtools", "/paperwork", "/paperwork/about", "/paperwork/terms"].includes(path)) return path;
...
export function createAnalytics(browser: AnalyticsWindow, id: string | null) {
  ...
  } catch { /* Storage may be blocked; the current tab still respects the choice. */ }
  ...
  function send(...args: unknown[]) {
    try { browser.gtag?.(...args); } catch { /* Analytics must never interrupt a tool. */ }
  }

  function context() {
    const path = publicPath(browser.location.pathname)!;
    return { page_location: `${browser.location.origin}${path}`, page_referrer: pageReferrer, page_title: "SmartTools" };
  }
  ...
  function pageView() {
    try {
      ...
      if (!script) {
        ...
        send("consent", "default", { analytics_storage: "granted", ad_storage: "denied", ad_user_data: "denied", ad_personalization: "denied" });
        send("js", new Date());
        send("config", validId, { ...context(), send_page_view: false, allow_google_signals: false, allow_ad_personalization_signals: false, cookie_domain: "none", cookie_expires: 60 * 60 * 24 * 180 });
        script = browser.document.createElement("script");
        script.async = true;
        script.crossOrigin = "anonymous";
        script.src = `https://www.googletagmanager.com/gtag/js?id=${validId}`;
        script.onload = () => { loaded = true; pageView(); };
        script.onerror = () => { loaded = false; if (browser.dataLayer) browser.dataLayer.length = 0; };
      ...
      if (loaded && lastPath !== path) {
        ...
        send("consent", "update", { analytics_storage: "granted", ad_storage: "denied", ad_user_data: "denied", ad_personalization: "denied" });
        send("config", validId, { ...context(), send_page_view: false, allow_google_signals: false, allow_ad_personalization_signals: false, cookie_domain: "none", cookie_expires: 60 * 60 * 24 * 180 });
  ...
  function setConsent(next: AnalyticsConsent, persist = true) {
    ...
    let saved = true;
    if (persist) {
      try { browser.localStorage.setItem(CONSENT_KEY, next ?? ""); } catch { saved = false; }
    }
    if (next !== "accepted" && validId) {
      ...
      if (script) send("consent", "update", { analytics_storage: "denied", ad_storage: "denied", ad_user_data: "denied", ad_personalization: "denied" });
      // These cookies are host-only (cookie_domain: none).
      try {
        for (const cookie of browser.document.cookie.split(";")) {
          const name = cookie.trim().split("=", 1)[0];
          if (name === "_ga" || name.startsWith("_ga_")) browser.document.cookie = `${name}=; Max-Age=0; Path=/; SameSite=Lax`;
        }
      } catch { /* Collection is disabled even when cookie access is blocked. */ }
    }
    pageView();
    return saved;
  }
  ...
  return {
    ...
    track(event: ToolEvent, toolKey?: string) {
      if (!allowed() || !loaded || !["tool_start", "tool_complete", "tool_error", "result_download", "result_copy"].includes(event)) return;
...
export function trackToolEvent(event: ToolEvent, toolKey?: string) {
  try { client?.track(event, toolKey); } catch { /* Never affect processing, copying or downloads. */ }
}
