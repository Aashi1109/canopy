...
import {
  and, asc, db, desc, eq, inArray, sql,
  blogPostsTable as posts, blogRevisionsTable as revisions,
  blogPostSchedulesTable as schedules, blogCategoriesTable as categories,
  blogTagsTable as tags, blogPublishedPostTagsTable as publishedTags, managedToolsTable,
} from '@smarttools/database';
import { requireTransactionPermission, writeAudit } from '../admin/adminMutations.ts';
import {
  assertBlogPublishable, blogDocumentHash, blogDocumentText, blogSlugFromTitle,
  createBlogDocument, validateBlogDocument, BlogValidationError, type BlogDocument,
} from './document.ts';
...
async function lockPost(tx: Transaction, input: z.infer<typeof postInput>, allowTrash = false) {
  ...
  if (post.version !== input.version) throw new BlogError('CONFLICT', 'This article changed. Reload the latest version before saving.');
...
async function resolveDocument(tx: Transaction, input: unknown, publishing = false): Promise<BlogDocument> {
  ...
  if (document.tags.length) {
    const terms = await tx.select().from(tags).where(inArray(tags.id, document.tags.map(tag => tag.id)));
    if (terms.length !== document.tags.length) throw new BlogError('VALIDATION', 'Choose existing tags.');
    document.tags = document.tags.map(tag => ({ id: tag.id, label: terms.find(term => term.id === tag.id)!.name }));
  }
  if (document.relatedToolIds.length) {
    const tools = await tx.select({ id: managedToolsTable.toolId }).from(managedToolsTable)
      .where(inArray(managedToolsTable.toolId, document.relatedToolIds));
    if (tools.length !== document.relatedToolIds.length) throw new BlogError('VALIDATION', 'Choose existing related tools.');
  }
...
async function checkpoint(tx: Transaction, post: Post, actor: string, reason: Revision['reason'], now: Date, sourceRevisionId?: string): Promise<Revision> {
  const [latest] = await tx.select().from(revisions).where(eq(revisions.postId, post.id))
    .orderBy(desc(revisions.revisionNumber)).limit(1);
  if (reason !== 'restore' && latest?.contentHash === post.draftHash) return latest;
  const [revision] = await tx.insert(revisions).values({
    id: randomUUID(), postId: post.id, revisionNumber: post.revisionSequence + 1,
    document: post.draftDocument, contentHash: post.draftHash, reason,
    sourceRevisionId, createdBy: actor, createdAt: now,
  }).returning();
  post.revisionSequence += 1;
  post.lastCheckpointAt = now;
  await tx.update(posts).set({ revisionSequence: post.revisionSequence, lastCheckpointAt: now }).where(eq(posts.id, post.id));
  return revision;
}

async function insertPost(tx: Transaction, actor: string, document: BlogDocument, duplicatedFrom?: string): Promise<Post> {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const [post] = await tx.insert(posts).values({
      id: randomUUID(), slug, draftDocument: document, draftHash: blogDocumentHash(document),
      createdBy: actor, draftUpdatedBy: actor,
    }).onConflictDoNothing({ target: posts.slug }).returning();
    if (!post) continue;
    await checkpoint(tx, post, actor, 'create', await databaseNow(tx));
    await writeAudit(tx, actor, duplicatedFrom ? 'blog.duplicate' : 'blog.create', 'blog_post', post.id, { duplicatedFrom, slug });
    return post;
  }
...
export async function createBlogPost(actor: string, input: unknown): Promise<Post> {
  const { title } = z.object({ title: z.string().trim().min(1).max(200) }).strict().parse(input);
...
export async function saveBlogDraft(actor: string, input: unknown): Promise<Post> {
  const value = postInput.extend({ document: z.unknown(), mode: z.enum(['autosave', 'manual']).default('autosave') }).strict().parse(input);
  return db.transaction(async tx => {
    ...
    if (changed) {
      Object.assign(post, { draftDocument: document, draftHash: hash, draftUpdatedAt: now, draftUpdatedBy: actor, updatedAt: now, version: post.version + 1 });
      await tx.update(posts).set({ draftDocument: document, draftHash: hash, draftUpdatedAt: now, draftUpdatedBy: actor, updatedAt: now, version: post.version }).where(eq(posts.id, post.id));
    }
    const before = post.revisionSequence;
    if (value.mode === 'manual' || (changed && (!post.lastCheckpointAt || now.getTime() - post.lastCheckpointAt.getTime() >= 60000))) {
      await checkpoint(tx, post, actor, value.mode === 'manual' ? 'manual_save' : 'autosave', now);
    }
    if (changed || before !== post.revisionSequence) {
      await writeAudit(tx, actor, 'blog.save', 'blog_post', post.id, { version: post.version, mode: value.mode, revisionNumber: post.revisionSequence });
    }
    return post;
  });
}
...
async function removeSchedule(tx: Transaction, postId: string, actor: string, reason: string) {
  ...
  for (const request of removed) await writeAudit(tx, actor, 'blog.schedule.cancel', 'blog_post', postId, {
    scheduleId: request.id, revisionId: request.revisionId, scheduledAt: request.scheduledAt.toISOString(), reason,
  });
  return removed.length > 0;
}

async function promote(tx: Transaction, post: Post, revision: Revision, actor: string, now: Date, schedule?: { id: string; scheduledAt: Date }): Promise<Post> {
  ...
  const [saved] = await tx.update(posts).set({
    publishedRevisionId: revision.id, publishedCategoryId: document.category!.id,
    ...
    version: post.version + 1, updatedAt: now,
  }).where(eq(posts.id, post.id)).returning();
  await tx.delete(publishedTags).where(eq(publishedTags.postId, post.id));
  if (document.tags.length) await tx.insert(publishedTags).values(document.tags.map(tag => ({ postId: post.id, tagId: tag.id })));
  await writeAudit(tx, actor, 'blog.publish', 'blog_post', post.id, { revisionId: revision.id, scheduleId: schedule?.id, scheduledAt: schedule?.scheduledAt.toISOString() });
  return saved;
}
...
export async function scheduleBlogPost(actor: string, input: unknown) {
  const value = postInput.extend({ scheduledAt: z.string().datetime({ offset: true }) }).strict().parse(input);
  return db.transaction(async tx => {
    await requireTransactionPermission(tx, actor, 'blog', 'publish');
    const post = await lockPost(tx, value);
    const now = await databaseNow(tx);
    ...
    const [schedule] = await tx.insert(schedules).values({ id: scheduleId, postId: post.id, revisionId: revision.id, scheduledAt, scheduledBy: actor, createdAt: now }).returning();
    const [saved] = await tx.update(posts).set({ version: post.version + 1, updatedAt: now }).where(eq(posts.id, post.id)).returning();
    await writeAudit(tx, actor, 'blog.schedule', 'blog_post', post.id, { scheduleId, revisionId: revision.id, scheduledAt: scheduledAt.toISOString() });
...
export async function retryBlogSchedule(actor: string, input: unknown): Promise<Post> {
  ...
  return db.transaction(async tx => {
    ...
    if (!candidate.scheduledBy) throw new AuthorizationError('Scheduling account no longer exists. Reschedule this article.');
    // Authorization rows precede the post lock, including the original publisher's rows.
    if (candidate.scheduledBy !== actor) await requireTransactionPermission(tx, candidate.scheduledBy, 'blog', 'publish');
    const post = await lockPost(tx, value);
    const [request] = await tx.select().from(schedules).where(and(eq(schedules.postId, post.id), eq(schedules.id, candidate.id))).for('update');
    ...
    const [revision] = await tx.select().from(revisions).where(and(eq(revisions.id, request.revisionId), eq(revisions.postId, post.id)));
...
async function lifecycle(actor: string, input: unknown, operation: 'cancel_schedule' | 'unpublish' | 'trash' | 'restore_trash'): Promise<Post> {
  const value = postInput.parse(input);
  return db.transaction(async tx => {
    await requireTransactionPermission(tx, actor, 'blog', operation === 'trash' || operation === 'restore_trash' ? 'archive' : 'publish');
    ...
    if (operation === 'trash' && post.publishedRevisionId) await requireTransactionPermission(tx, actor, 'blog', 'publish');
    const now = await databaseNow(tx);
    ...
    await writeAudit(tx, actor, `blog.${operation}`, 'blog_post', post.id, { version: saved.version });
    return saved;
  });
}
...
export async function restoreBlogRevision(actor: string, input: unknown): Promise<Post> {
  ...
  return db.transaction(async tx => {
    await requireTransactionPermission(tx, actor, 'blog', 'edit');
    const post = await lockPost(tx, value);
    const [source] = await tx.select().from(revisions).where(and(eq(revisions.id, value.revisionId), eq(revisions.postId, post.id)));
    ...
    Object.assign(post, { draftDocument: document, draftHash: blogDocumentHash(document), draftUpdatedBy: actor, draftUpdatedAt: now, version: post.version + 1, updatedAt: now });
    await tx.update(posts).set({ draftDocument: document, draftHash: post.draftHash, draftUpdatedBy: actor, draftUpdatedAt: now, version: post.version, updatedAt: now }).where(eq(posts.id, post.id));
    const restored = await checkpoint(tx, post, actor, 'restore', now, source.id);
    await writeAudit(tx, actor, 'blog.restore_revision', 'blog_post', post.id, { sourceRevisionId: source.id, revisionId: restored.id });
    return post;
  });
}

export async function saveBlogTerm(actor: string, input: unknown) {
  const value = z.object({ kind: z.enum(['category', 'tag']), id: identity.optional(), name: z.string().trim().min(1).max(100).refine(name => name.isWellFormed() && !/[\u0000-\u001f\u007f]/u.test(name), 'Use a name without control characters.') }).strict().parse(input);
  ...
  return db.transaction(async tx => {
    ...
    if (value.id) {
      ...
      const [conflict] = await tx.select({ id: table.id }).from(table).where(sql`lower(${table.name}) = lower(${value.name}) AND ${table.id} <> ${value.id}`);
      if (conflict) throw new BlogError('CONFLICT', 'A category or tag with this name already exists.');
      const [saved] = await tx.update(table).set({ name: value.name, updatedBy: actor, updatedAt: await databaseNow(tx) }).where(eq(table.id, value.id)).returning();
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      const slug = attempt === 0 ? base : `${base.slice(0, 151).replace(/-+$/, '')}-${randomBytes(4).toString('hex')}`;
      const [saved] = await tx.insert(table).values({ id: randomUUID(), name: value.name, slug, createdBy: actor, updatedBy: actor }).onConflictDoNothing().returning();
      if (saved) {
        await writeAudit(tx, actor, `blog.${value.kind}.create`, `blog_${value.kind}`, saved.id);
        return saved;
      }
      const [conflict] = await tx.select({ id: table.id }).from(table).where(sql`lower(${table.name}) = lower(${value.name})`);
      if (conflict) throw new BlogError('CONFLICT', 'A category or tag with this name already exists.');
    }
...
export async function publishDueBlogPosts() {
  ...
  while (Date.now() - started < 20000) {
    const candidates = await db.select({ ...getTableColumns(schedules), cursorTime: sql<string>`to_char(${schedules.scheduledAt} AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"')` }).from(schedules).where(and(
      sql`${schedules.scheduledAt} <= ${cutoff}`,
      cursor ? sql`(${schedules.scheduledAt}, ${schedules.id}) > (${cursor.at}::timestamptz, ${cursor.id})` : undefined,
    )).orderBy(asc(schedules.scheduledAt), asc(schedules.id)).limit(50);
    ...
    for (const candidate of candidates) {
      ...
      try {
        const published = await db.transaction(async tx => {
          ...
          const [post] = await tx.select().from(posts).where(eq(posts.id, candidate.postId)).for('update', { skipLocked: true });
        ...
        result.failed++;
        const code = error instanceof AuthorizationError ? 'PUBLISHER_FORBIDDEN' : error instanceof BlogValidationError || error instanceof BlogError ? 'INVALID_CONTENT' : 'TEMPORARY_FAILURE';
        // Identity, not post ID: a failed old request must never poison a replacement.
        try {
          await db.update(schedules).set({ lastAttemptAt: sql`clock_timestamp()`, lastErrorCode: code }).where(eq(schedules.id, candidate.id));
        } catch {
          console.warn('blog.schedule.diagnostics_failed', { postId: candidate.postId, scheduleId: candidate.id, code });
        }
      }
    }
    if (candidates.length < 50) break;
  }
  const [remaining] = await db.select({ count: sql<number>`count(*)::integer` }).from(schedules).where(sql`${schedules.scheduledAt} <= ${cutoff}`);
