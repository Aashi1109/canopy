...
import { BlogAssistantError } from "./assistantValidation.ts";
import { BlogValidationError } from "./document.ts";
...
export async function readAssistantJson(request: Request): Promise<unknown> {
  ...
  try {
    ...
    throw new BlogAssistantError("VALIDATION", "Send a valid JSON request.");
  }
}
...
async function boundedRequestBytes(request: Request, maximum: number): Promise<Buffer> {
  if (Number(request.headers.get("content-length")) > maximum)
    throw new BlogAssistantError("VALIDATION", "Request is too large.", 413);
...
export function assistantErrorResponse(error: unknown): Response {
  ...
  if (error instanceof BlogValidationError)
    return Response.json({ code: "VALIDATION", error: error.message }, { status: 400, headers });
  if (error instanceof BlogAssistantError || error instanceof AIError)
...
export async function blogAssistantRoute(
  request: Request,
  ...
  try {
    if (!["GET", "HEAD"].includes(request.method) && request.headers.get("origin") !== new URL(request.url).origin)
      throw new BlogAssistantError("FORBIDDEN", "Invalid request origin.", 403);
    const session = await getSession(request.headers);
    if (!session) throw new BlogAssistantError("UNAUTHENTICATED", "Sign in to continue.", 401);
    if (session.user.status !== "active")
      throw new BlogAssistantError("FORBIDDEN", "This account cannot use the assistant.", 403);
