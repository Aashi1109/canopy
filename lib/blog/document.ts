...
export interface BlogTerm { id: string; label: string }
...
type NodeType = "taskList" | "taskItem" | "doc" | "paragraph" | "text" | "heading" | "bulletList" | "orderedList" | "listItem" | "blockquote" | "horizontalRule" | "hardBreak" | "table" | "tableRow" | "tableCell" | "tableHeader" | "codeBlock" | "image";
...
export interface BlogDocumentOptions { cloudName?: string }

export class BlogValidationError extends Error {
  constructor(message: string) { super(message); this.name = "BlogValidationError"; }
}

function fail(message: string): never { throw new BlogValidationError(message); }
function record(input: unknown, label: string): Record<string, unknown> {
  if (!input || typeof input !== "object" || Array.isArray(input) || ![Object.prototype, null].includes(Object.getPrototypeOf(input))) fail(`${label} must be a JSON object.`);
  return input as Record<string, unknown>;
}
function keys(input: Record<string, unknown>, allowed: readonly string[], label: string) {
  for (const key of Object.keys(input)) if (!allowed.includes(key)) fail(`${label} has an unsupported property: ${key}.`);
}
function string(input: unknown, label: string, max: number, trim = true): string {
  if (typeof input !== "string" || !input.isWellFormed() || input.length > max || /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(input)) fail(`${label} must be text of at most ${max} characters without control characters.`);
  return trim ? input.trim() : input;
}
function integer(input: unknown, label: string, min: number, max: number): number {
  if (typeof input !== "number" || !Number.isSafeInteger(input) || input < min || input > max) fail(`${label} must be an integer from ${min} to ${max}.`);
  return input;
}
  ...
  function walk(value: unknown, depth: number) {
    ...
    if (typeof value === "string") {
      ...
      if (Array.isArray(value)) {
        ...
        if (Object.getPrototypeOf(value) !== Array.prototype || Reflect.ownKeys(value).length !== value.length + 1) fail("Document lists must contain only JSON items.");
        ...
        if (Reflect.ownKeys(object).length !== Object.keys(object).length) fail("Document must contain only JSON properties.");
        for (const key of Object.keys(object)) {
          if (["__proto__", "prototype", "constructor"].includes(key)) fail("Document contains an unsafe JSON property.");
    ...
    } else if (value !== null && typeof value !== "boolean" && !(typeof value === "number" && Number.isFinite(value))) fail("Document must contain only JSON values.");
  }
  walk(input, 0);
  if (new TextEncoder().encode(JSON.stringify(input)).byteLength > 1024 * 1024) fail("Document exceeds the 1 MiB size limit.");
}
...
export function blogImageUrl(image: BlogImage, options: BlogDocumentOptions): string {
  if (!options.cloudName || !/^[a-zA-Z0-9_-]+$/.test(options.cloudName)) fail("A configured Cloudinary cloud is required for blog images.");
...
export function validateBlogImage(input: unknown, options: BlogDocumentOptions = {}): BlogImage {
  ...
  if (!/^smarttools\/blog\/[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(publicId)) fail("Image public ID must identify an uploaded immutable blog asset.");
  if (!["jpg", "jpeg", "png", "webp"].includes(object.format as string)) fail("Image format must be JPEG, PNG or WebP.");
  const image: BlogImage = {
    publicId, version: integer(object.version, "Image version", 1, Number.MAX_SAFE_INTEGER),
  ...
  if (object.src !== undefined && object.src !== url) fail("Image URL must match its immutable configured-cloud asset.");
  return image;
}
function safeLink(input: unknown): string {
  ...
  let url: URL;
  try { url = new URL(href); } catch { fail("Link URL is invalid."); }
  if (!["https:", "http:", "mailto:"].includes(url.protocol) || url.username || url.password) fail("Link URL uses an unsafe protocol or credentials.");
  return url.href;
}
function mark(input: unknown): BlogMark {
  const object = record(input, "Text mark");
  keys(object, ["type", "attrs"], "Text mark");
  if (!["bold", "italic", "strike", "underline", "code", "link", "highlight", "superscript", "subscript"].includes(object.type as string)) fail("Text mark is unsupported.");
  if (object.type === "highlight") {
    ...
    if (typeof attrs.color !== "string" || !/^#[0-9a-f]{6}$/i.test(attrs.color)) fail("Highlight color must be a six-digit hexadecimal color.");
    return { type: "highlight", attrs: { color: attrs.color.toLowerCase() } };
  }
  if (object.type !== "link") {
    if (object.attrs !== undefined && Object.keys(record(object.attrs, "Mark attributes")).length) fail("Text mark has unsupported attributes.");
  ...
  if (attrs.class != null || attrs.title != null || (attrs.target != null && attrs.target !== "_blank" && attrs.target !== "_self")) fail("Link has unsupported attributes.");
  if (attrs.rel != null && (typeof attrs.rel !== "string" || !/^(?:(?:noopener|noreferrer|nofollow)\s*)*$/.test(attrs.rel))) fail("Link relationship is unsupported.");
  const target = attrs.target === "_self" ? "_self" : "_blank";
  const rel = `${target === "_blank" ? "noopener noreferrer" : ""}${typeof attrs.rel === "string" && attrs.rel.split(/\s+/).includes("nofollow") ? " nofollow" : ""}`.trim();
  return { type: "link", attrs: { href: safeLink(attrs.href), target, rel } };
}

const BLOCKS: NodeType[] = ["taskList", "paragraph", "heading", "bulletList", "orderedList", "blockquote", "horizontalRule", "table", "codeBlock", "image"];
const CHILDREN: Partial<Record<NodeType, readonly NodeType[]>> = {
  doc: BLOCKS, paragraph: ["text", "hardBreak"], heading: ["text", "hardBreak"],
  bulletList: ["listItem"], orderedList: ["listItem"], listItem: BLOCKS,
  taskList: ["taskItem"], taskItem: BLOCKS,
  blockquote: BLOCKS, table: ["tableRow"], tableRow: ["tableCell", "tableHeader"],
  tableCell: BLOCKS, tableHeader: BLOCKS, codeBlock: ["text"],
};
...
function validateTable(node: BlogNode) {
  ...
  if (!width || occupied.some((row) => row.length !== width || Array.from({ length: width }, (_, x) => row[x]).some((value) => !value))) fail("Table rows must form a complete rectangular grid.");
  return columnWidths;
}
...
function validateBody(input: unknown, options: BlogDocumentOptions): BlogNode {
  ...
  function visit(value: unknown, depth: number): BlogNode {
    ...
    if (!["doc", "text", "taskItem", "listItem", "hardBreak", "tableRow", "tableCell", "tableHeader", ...BLOCKS].includes(type)) fail("Editor node type is unsupported.");
    ...
    if (type === "text") {
      ...
      if (object.marks !== undefined) {
        ...
        if (new Set(marks.map((item) => item.type)).size !== marks.length) fail("Duplicate text marks are unsupported.");
        if (marks.some((item) => item.type === "code") && marks.length > 1) fail("Inline code cannot combine with other text marks.");
    ...
    if (type === "heading" || type === "paragraph") {
      ...
      if (attrs.textAlign != null) {
        if (!["left", "center", "right", "justify"].includes(attrs.textAlign as string)) fail("Text alignment is unsupported.");
      ...
      node.attrs = { start: attrs.start === undefined ? 1 : integer(attrs.start, "List start", 1, 1000000) };
      ...
      if (alignment != null && !["left", "center", "right"].includes(alignment as string)) fail("Image alignment is unsupported.");
      node.attrs = {
      ...
      const colwidth = attrs.colwidth == null ? null : list(attrs.colwidth, "Column widths", colspan).map((width) => integer(width, "Column width", 0, 10000));
      ...
      if (attrs.backgroundColor != null) {
        if (typeof attrs.backgroundColor !== "string" || !/^#[0-9a-f]{6}$/i.test(attrs.backgroundColor)) fail("Table cell background color must be a six-digit hexadecimal color.");
    ...
    if (!["doc", "paragraph", "heading", "codeBlock", "tableRow"].includes(type) && !node.content.length) fail(`${type} must contain content.`);
    if (["listItem", "taskItem"].includes(type) && node.content[0]?.type !== "paragraph") fail("List items must start with a paragraph.");
    if (type === "codeBlock" && node.content.some((child) => child.marks?.length)) fail("Code blocks cannot contain text marks.");
...
export function validateBlogDocument(input: unknown, options: BlogDocumentOptions = {}): BlogDocument {
  ...
  keys(object, ["schemaVersion", "title", "excerpt", "body", "coverImage", "authorName", "category", "tags", "seoTitle", "seoDescription", "relatedToolIds"], "Article document");
  if (object.schemaVersion !== 1) fail("Article schema version is unsupported.");
  const tags = list(object.tags, "Tags", 20).map(term).sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0);
  ...
  return {
    schemaVersion: 1, title: string(object.title, "Title", 200), excerpt: string(object.excerpt, "Excerpt", 500),
    body: validateBody(object.body, options), coverImage: object.coverImage === null ? null : validateBlogImage(object.coverImage, options),
    authorName: string(object.authorName, "Author name", 150), category: object.category === null ? null : term(object.category), tags,
    seoTitle: object.seoTitle === null ? null : string(object.seoTitle, "SEO title", 160) || null,
    seoDescription: object.seoDescription === null ? null : string(object.seoDescription, "SEO description", 320) || null,
    relatedToolIds,
  };
}

export function createBlogDocument(title: string): BlogDocument {
  const document = validateBlogDocument({ schemaVersion: 1, title, excerpt: "", body: { type: "doc", content: [{ type: "paragraph" }] }, coverImage: null, authorName: "SmartTools Team", category: null, tags: [], seoTitle: null, seoDescription: null, relatedToolIds: [] });
...
function nodeText(node: BlogNode): string {
  ...
  return ["doc", "paragraph", "heading", "codeBlock", "tableCell", "tableHeader"].includes(node.type) ? `${text}\n` : text;
}
export function blogDocumentText(document: BlogDocument): string { return nodeText(document.body).trim(); }
...
export function assertBlogPublishable(document: BlogDocument): void {
  ...
  function check(node: BlogNode) {
    if (node.type === "image" && !String(node.attrs?.alt ?? "").trim()) fail("Inline image alt text is required for publication.");
...
export function blogSlugFromTitle(title: string): string {
  ...
  try { return slugFromName(normalized).slice(0, 160).replace(/-+$/, ""); }
  catch { return `post-${randomBytes(4).toString("hex")}`; }
}
...
export function renderBlogDocument(input: BlogDocument, options: BlogDocumentOptions = {}): { html: string; headings: { id: string; level: number; text: string }[]; readingMinutes: number } {
  ...
  function render(node: BlogNode): string {
    ...
    switch (node.type) {
      ...
      case "text": {
        ...
        for (const textMark of node.marks ?? []) {
          if (textMark.type === "link") {
            ...
            const tag = { bold: "strong", italic: "em", strike: "s", underline: "u", code: "code", highlight: "mark", superscript: "sup", subscript: "sub" }[textMark.type];
            html = `<${tag}>${html}</${tag}>`;
          }
        }
        return html;
      }
      case "paragraph": return `<p${alignment}>${content()}</p>`;
      case "heading": {
      ...
      case "taskItem": return `<li data-type="taskItem"><input type="checkbox" disabled${node.attrs?.checked ? " checked" : ""} aria-label="${node.attrs?.checked ? "Completed" : "Not completed"}"><div>${content()}</div></li>`;
      case "bulletList": return `<ul>${content()}</ul>`;
      case "orderedList": return `<ol start="${node.attrs!.start}">${content()}</ol>`;
      case "listItem": return `<li>${content()}</li>`;
      case "blockquote": return `<blockquote>${content()}</blockquote>`;
      case "horizontalRule": return "<hr>";
      case "hardBreak": return "<br>";
      case "codeBlock": return `<pre><code${node.attrs?.language ? ` class="language-${escapeHtml(node.attrs.language)}"` : ""}>${content()}</code></pre>`;
      case "table": {
        ...
        const sizing = resized ? `;table-layout:fixed;${widths.every(Boolean) ? `width:${totalWidth}px` : `width:100%;min-width:${totalWidth}px`}` : "";
        const columns = resized ? `<colgroup>${widths.map((width) => width ? `<col style="width:${width}px">` : "<col>").join("")}</colgroup>` : "";
      ...
      case "tableCell": case "tableHeader": {
        ...
        const styles = [node.attrs!.align ? `text-align:${node.attrs!.align}` : "", node.attrs!.backgroundColor ? `background-color:${node.attrs!.backgroundColor}` : ""].filter(Boolean).join(";");
  ...
  return { html, headings, readingMinutes: Math.max(1, Math.ceil(blogDocumentText(document).split(/\s+/).filter(Boolean).length / 200)) };
}
