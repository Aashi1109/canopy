...
import { attachmentProvider, deleteRunResponses } from "./assistantThreads.ts";

/** Expire private content/resources, retaining execution and usage records for reporting. */
export async function cleanupBlogAssistant() {
          ...
          await tx
            ...
            .where(
              and(
                ...
                sql`NOT EXISTS (SELECT 1 FROM blog_runs r WHERE r.input_message_id = ${locked.inputMessageId} AND r.expires_at > ${now})`,
              ),
            );
        return locked;
      });
  ...
  // Cleanup retained legacy provider handles; completion delivery never polls here.
  ...
  await db
    .delete(threads)
    .where(
      sql`${threads.updatedAt} <= now() - interval '30 days' AND NOT EXISTS (SELECT 1 FROM blog_runs WHERE thread_id = ${threads.id}) AND NOT EXISTS (SELECT 1 FROM blog_attachments WHERE thread_id = ${threads.id}) AND NOT EXISTS (SELECT 1 FROM blog_messages WHERE thread_id = ${threads.id})`,
    );
  return counts;
}
